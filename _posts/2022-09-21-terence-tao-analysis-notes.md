...
本文内容主要来自[crasysky的cnblogs](https://www.cnblogs.com/crasysky/p/16367673.html)，在该基础上做了一些修正和补充。再次感谢原博主的辛勤劳动。
...
- **强归纳原理**：令$$ m_0 $$为自然数，若$$ \forall m \ge m_0,\forall m_0\le m'\lt m $$均有$$ P(m') $$成立，则$$ P(m) $$也为真。 那么对于任何$$ m\ge m_0 $$均有$$ P(m) $$为真。
...
- **公理7(无穷公理)**：存在集合$$ \mathbb{N} $$，对象$$ 0\in \mathbb{N} $$，且对每个$$ n\in \mathbb{N} $$，$$ n++ $$是对象，且$$ n++\in \mathbb{N} $$
...
有序n元组可利用函数定义：对任意集合$$ X $$，满射函数$$ x:\{i\in \mathbb{N}:1\le i\le n\}\to X $$，令$$ x_i=x(i) $$并用函数相等的定义证明n元组相等的定义。利用这个定义可证明多重笛卡尔积的结果是集合：令$$ X=\bigcup X_i $$，利用幂集公理可知存在集合$$ A=X^{\{i\in \mathbb{N}:1\le i\le n\}} $$，利用分类公理有$$ \{f\in A:\forall 1\le i\le n有f(i)\in X_i\} $$。
...
- **命题**：自然数系唯一。如果存在另一个满足皮亚诺公理的自然数系$$ \mathbb{N}' $$，存在双射$$ a:\mathbb{N}\to \mathbb{N}' $$满足$$ a(0)=0' $$且$$ \forall n\in \mathbb{N},n'\in \mathbb{N}' $$,$$ a(n)=n' $$当且仅当$$ a(n++)=n'++ $$。

首先证明引理：对$$ f:\mathbb{N}\times \mathbb{N}\to \mathbb{N} $$和自然数$$ c $$，存在函数$$ a:\mathbb{N}\to \mathbb{N} $$满足$$ a(0)=c $$且$$ \forall n\in \mathbb{N} $$有$$ a(n++)=f(n,a(n)) $$。

使用类似有限选取引理的证明过程归纳：设已经存在唯一函数$$ a_N:\{n\in \mathbb{N}:n\le N\}\to \mathbb{N} $$，如果只使用皮亚诺公理和集合论可以归纳得到集合$$ \{n\in \mathbb{N}:n\le N\} $$。

那么只需要令$$ f(n,a(n))=a(n)++ $$，$$ c=0' $$，则有$$ a(n++)=a(n)++ $$。若$$ a(n)=n' $$，则$$ a(n++)=n'++ $$；若$$ a(n++)=n'++ $$，$$ a(n)++=n'++ $$，由于$$ \mathbb{N}' $$也满足皮亚诺公理，根据公理4可知$$ a(n)=n' $$。
...
- **基数的定义**：称集合$$ X $$的基数为自然数$$ n $$，当且仅当$$ X $$与集合$$ \{i\in \mathbb{N}:1\le i\le n\} $$基数相等。

- **引理**：若$$ X $$的基数为$$ n\ge 1 $$，则$$ X\ne \emptyset $$，且存在$$ x\in X $$，$$ X\setminus \{x\} $$的基数为$$ n-1 $$。(暂且定义$$ n-1 $$为满足$$ m++=n $$的唯一自然数)

可直接构造证明：设$$ f:X\to \{i\in \mathbb{N}:1\le i\le n\} $$，令$$ f(x')\lt f(x) $$时$$ g(x')=f(x') $$，$$ f(x')\gt f(x) $$时$$ g(x')=f(x')-1 $$即可，再证明是双射即可。

再证明基数的唯一性：对$$ n $$归纳使用引理即可。

- **集合有限的定义**：集合有限定义为基数是自然数，将有限集$$ X $$的基数记做$$ \#(X) $$。

证明自然数集不是有限集：反证，设$$ f:\{i\in \mathbb{N}:1\le i\le N\}\to \mathbb{N} $$，归纳得到存在$$ f(i)\le M $$对任意$$ i $$成立。
...
由于基数算数定理的证明并没有用到自然数的算数定理，可以利用基数来证明自然数的算术定理：将$$ n $$写成集合$$ \{x\in \mathbb{N}:1\le x\le n\} $$的基数，构造双射可知$$ \#(A\times B)=\#(B\times A) $$，那么$$ a\times b=\#(A)\times \#(B)=\#(A\times B)=\#(B\times A)=\#(B)\times \#(A)=b\times a $$，就证明了乘法交换律。
...
- **序列的定义**：设$$ m $$是整数，序列$$ (a_n)_{n=m}^{\infty} $$是从集合$$ \{n\in Z:n\ge m\} $$到$$ \mathbb{Q} $$的函数，对每个大于等于$$ m $$的整数$$ n $$指定有理数$$ a_n $$。
...
如果要证明必然存在无理数$$ z $$满足$$ x\lt z\lt y $$，可以两次使用命题3，找到两个有理数$$ q_1 $$和$$ q_2 $$满足$$ x\lt q_1\lt q_2\lt y $$，$$ q1+(q2-q1)/\sqrt{2} $$是无理数可通过有理数的运算结果是有理数反证得到。（网上还有证法：$$ X=x+(y-x)/2 $$,$$ Y=x+(y-x)/\sqrt{2} $$,$$ x+(y-x)/\sqrt{3} $$必有一个为无理数，否则$$ (X-Y)/(X-Z) $$是有理数，但是这里需要证明$$ \sqrt{3} $$也是无理数）
...
- **上界的定义**：设$$ E $$是$$ \mathbb{R} $$的一个子集，实数$$ M $$是$$ E $$的上界当且仅当$$ \forall x\in E $$都有$$ x\le M $$。

注意并不能说$$ E $$中最大元素是上界，$$ E $$是无限集，且很可能不存在最大值如$$ {x\in \mathbb{R}:1\lt x\lt 2} $$

- **最小上界的定义**：$$ M $$是最小上界当且仅当$$ M $$是上界且对所有上界$$ M' $$满足$$ M'\ge M $$。

- **定理(最小上界存在性)**：如果$$ E $$是$$ \mathbb{R} $$的非空子集且$$ E $$存在上界，则恰有一个最小上界。
...
- **上确界的定义**：设$$ E $$是$$ \mathbb{R} $$的非空子集且存在上界，定义$$\sup(E) $$为$$ E $$的最小上界，如果$$ E $$非空且没有上界，令$$\sup(E):=+\infty $$；如果$$ E $$为空，令$$\sup(E):=-\infty $$。$$\sup(E) $$称为上确界。

同理我们也可以定义下确界$$ \inf(E) $$。

- **命题**：存在一个实数$$ x $$满足$$ x^2=2 $$。

证明（巧妙！）：思路是利用上确界。令$$ E={y\in \mathbb{R}:y\ge 0且y^2\lt 2} $$，显然$$ 2 $$是$$ E $$的上界，因此存在$$ x=\sup(E) $$，利用实数的三歧性。
...
- **n次根的定义**：对非负实数$$ x $$和正整数$$ n $$，$$ x^{1/n}:=\sup\{y\in \mathbb{R}:y\ge 0且y^n\le x\} $$。
...
- **广义实数系$$ \mathbb{R}^{\ast} $$的定义**：实数$$ \mathbb{R} $$附加上$$ +\infty $$和$$ -\infty $$两个元素，广义实数有限当且仅当它是实数，无限当且仅当它是$$ +\infty $$或$$ -\infty $$。
...
- **广义实数集的上确界的定义**：设$$ E $$是$$ \mathbb{R}^{\ast} $$的子集，定义$$\sup(E) $$:(a)若$$ E $$是$$ \mathbb{R} $$的子集，按照之前的定义；(b)如果$$ E $$包含$$ +\infty $$，则$$\sup(E):=+\infty $$；(c)如果$$ E $$包含$$ -\infty $$，则$$\sup(E):=\sup(E\setminus \{-\infty\}) $$。
...
- **子序列的定义**：设$$ (a_n)_{n=0}^{\infty} $$和$$ (b_n)_{n=0}^{\infty} $$，称$$ (b_n)_{n=0}^{\infty} $$是$$ (a_n)_{n=0}^{\infty} $$的子序列当且仅当存在严格递增的函数$$ f:\mathbb{N}\to \mathbb{N} $$满足$$ b_n=a_{f(n)} $$对所有$$ n\in \mathbb{N} $$成立。

- **定理1**：序列收敛于$$ L $$当且仅当所有子序列均收敛于$$ L $$
- **定理2**：序列存在极限点$$ L $$当且仅当存在子序列收敛于$$ L $$

定理2的左蕴含右证明用类似实数的上界存在性的证法（因为是先构造序列，再证明对所有$$ \varepsilon $$成立，所以利用$$ \lim_{n\to \infty}1/n=0 $$）。根据$$ L $$为极限点，令$$ \varepsilon=1/n $$可得集合$$ \{m\in \mathbb{N}:m\gt f(n-1)且d(a_m,L)\le 1/n\} $$不为空，令$$ f(n) $$为集合中任一元素即可。
...
- **有限集求和运算的定义**：设$$ X $$是基数为$$ n $$的有限集，则存在双射$$ g:\{i\in \mathbb{N}:1\le i\le n\}\to X $$，设$$ f:X\to \mathbb{R} $$，则$$ \sum_{x\in X}f(x):=\sum_{i=1}^{n}f(g(i)) $$
...
- **引理**：若$$ X $$和$$ Y $$是有限集，设函数$$ f:X\times Y\to \mathbb{R} $$，则$$ \sum_{x\in X}(\sum_{y\in Y}f(x,y))=\sum_{(x,y)\in X\times Y}f(x,y) $$
...
- **命题1**：如果$$ \sum_{n=0}^{\infty}a_n $$是收敛的非负实数的级数，$$ f:\mathbb{N}\to \mathbb{N} $$是双射，则$$ \sum_{m=0}^{\infty}a_{f(m)} $$也收敛于相同的和。

证明：令两者的部分和分别是$$ S_N $$和$$ T_M $$，根据序列上下界命题1可转化为证明$$\sup(S_N)=\sup(T_M) $$。令$$ Y=\{m\in \mathbb{N}:m\le M\} $$，分别根据有限集求和定理2和有限集求和定理1有$$ T_M=\sum_{m\in Y}a_{f(m)}=\sum_{n\in f(Y)}a_n $$，序列$$ (f(m))_{m=0}^{M} $$有限因此有界，存在$$ N\ge f(m) $$对所有$$ m\le M $$成立，再分别根据有限集求和定理3和有限集求和定理2可知$$ T_M\le \sum_{n\in \{n\in \mathbb{N}:n\le N\}}a_n=S_N\le\sup(S_N)_{N=0}^{\infty} $$，因此$$\sup(T_M)_{M=0}^{\infty}\le\sup(S_N)_{N=0}^{\infty} $$。用$$ f^{-1} $$同理可得$$\sup(T_M)\ge\sup(S_N) $$。

- **命题2**：如果$$ \sum_{n=0}^{\infty}a_n $$是绝对收敛的实数级数，$$ f:\mathbb{N}\to \mathbb{N} $$是双射，则$$ \sum_{m=0}^{\infty}a_{f(m)} $$也绝对收敛，且收敛于相同的和。

根据命题1可知其绝对收敛。证明收敛于相同的和思路是利用级数收敛的判定定理。令两者的部分和分别是$$ S_N $$和$$ T_M $$，$$ S $$收敛于$$ L $$，对于$$ \varepsilon' $$，可以找到$$ N $$满足$$ p,q\ge N $$时有$$ \sum_{n=p}^{q}\vert a_n\vert \le \varepsilon' $$。$$ S_N $$是$$ \varepsilon' $$-接近$$ L $$的，类似命题1的办法，存在$$ M\ge f^{-1}(n) $$对所有$$ n\le N $$成立，利用有限集求和定理1,2和3有$$ \sum_{m=0}^{M}a_{f(m)}=\sum_{n\in \{f(m):m\in \mathbb{N},m\le M\}}a_n=S_N+\sum_{n\in X}a_n $$，其中$$ X=f(M)\setminus \{n\in \mathbb{N}:n\le N\} $$。$$ X $$有限因此存在上界$$ q $$，$$ X\subseteq \{n\in \mathbb{N}:N+1\le n\le q\} $$，根据有限级数定理5和有限集求和定理3可得$$ \vert \sum_{n\in X}a_n\vert \le \sum{n\in X}\vert a_n\vert \le \sum_{n=N+1}^{q}\vert a_n\vert \le \varepsilon' $$，因此当$$ M'\ge M $$时$$ T_{M'} $$是$$ \varepsilon' $$-接近于$$ S_N $$的，即$$ T $$是最终$$ 2\varepsilon' $$-接近于$$ L $$的。
...
- **可数集的定义**：集合$$ X $$是可数的，当且仅当$$ X $$与自然数集$$ \mathbb{N} $$有相等的基数(存在双射)；$$ X $$是至多可数的，当且仅当$$ X $$是可数的或$$ X $$是有限的；$$ X $$是不可数的，当且仅当$$ X $$不是可数的也不是有限的

- **定理(良序原理)**：设$$ X $$是$$ \mathbb{N} $$的非空子集，恰存在一个元素$$ n\in X $$满足对所有$$ m\in X $$有$$ n\le m $$（自然数集的非空子集存在最小元素）
...
- **命题1**：如果$$ X $$是$$ \mathbb{N} $$的无限子集，那么存在唯一递增的双射$$ f:\mathbb{N}\to X $$
...
证明：$$ X $$有限可根据基数算术定理2得到结论；$$ X $$无限根据定义可知存在$$ f:X\to \mathbb{N} $$，那么$$ f:Y\to f(Y) $$也是双射，$$ Y $$与$$ f(Y) $$基数相同，而$$ f(Y)\subseteq \mathbb{N} $$，由上述命题1可知$$ f(Y) $$可数，$$ f(Y) $$与$$ \mathbb{N} $$有相等的基数。

- **推论2**：$$ A $$是至多可数的当且仅当存在$$ A\to \mathbb{N} $$的单射。

证明：如果$$ A $$可数，利用可数的定义，如果$$ A $$有限，利用集合基的定义；如果存在单射，则存在$$ A\to f(A) $$的双射，根据推论1可知$$ f(A) $$至多可数。

- **命题2**：$$ Y $$是集合，设$$ f:\mathbb{N}\to Y $$，则$$ f(\mathbb{N}) $$至多可数

证明：思路是找到集合$$ X\subseteq \mathbb{N} $$且$$ f:X\to f(\mathbb{N}) $$构成双射，再利用命题1得到$$ X $$至多可数。令$$ X=\{x\in \mathbb{N}:\forall x'\lt x有f(x)\ne f(x')\} $$即可。

- **推论**：$$ X $$是可数集，$$ Y $$是集合，设$$ f:X\to Y $$，则$$ f(X) $$至多可数

证明：存在双射$$ g:\mathbb{N}\to X $$，则$$ f(x)=f(g(g^{-1}(x))) $$，则$$ f(X)=f(g(f^{-1}(X)))=f(g(\mathbb{N})) $$，利用命题2可知$$ (f\circ g)(\mathbb{N}) $$至多可数。

- **命题3**：$$ X $$和$$ Y $$是可数集，则$$ X\cup Y $$是可数集

证明：考虑如何构造$$ \mathbb{N}\to Z $$的双射，应当$$ f(0)=0,f(1)=1,f(2)=-1,f(3)=2,f(4)=-2,...,f(2k)=-k,f(2k+1)=k+1,... $$。设存在双射$$ g:\mathbb{N}\to X $$,$$ h:\mathbb{N}\to Y $$，那么令$$ g(n)=f(2n),h(n)=f(2n+1) $$，则$$ f(\mathbb{N})=X\cup Y $$，由命题2可知$$ X\cup Y $$可数。

- **命题4**：集合$$ A=\{(n,m)\in \mathbb{N}\times \mathbb{N}:m\le n\} $$是可数集

证明：思路是按照一定顺序“分配”给自然数$$ (1,1),(2,1),(2,2),(3,1),(3,2),(3,3),... $$。令$$ a_0=0 $$,$$ a_{n+1}=a_n+n+1 $$，那么$$ a_N $$就表示$$ n\lt N $$的二元组个数，直接令$$ f(n,m):=a_n+m $$即可。只需说明$$ f $$是一对一的即可说明存在$$ A $$到$$ f(A) $$的双射，由命题1的推论1可知其至多可数，而如果$$ A $$有限，则任一个子集都有限，而$$ A'=\{(n,0)\in \mathbb{N}\times \mathbb{N}:n\in \mathbb{N}\} $$与$$ \mathbb{N} $$基数相同，矛盾，因此$$ A $$是可数的。（直接说明$$ f $$是满射似乎更方便）

- **推论1**：$$ \mathbb{N}\times \mathbb{N} $$是可数集

证明：令$$ B=\{(n,m)\in \mathbb{N}\times \mathbb{N}:m\ge n\} $$，则$$ A $$和$$ B $$基数相同，$$ B $$可数；而$$ \mathbb{N}\times \mathbb{N}=A\cup B $$，根据命题3可得结论。

- **推论2**：如果$$ X $$和$$ Y $$是可数集，则$$ X\times Y $$是可数集

证明：存在双射$$ g:X\to \mathbb{N} $$和$$ h:Y\to \mathbb{N} $$，则$$ f(x,y)=(g(x),h(y)) $$是$$ X\times Y\to \mathbb{N}\times \mathbb{N} $$的双射。

- **推论3**：有理数集$$ \mathbb{Q} $$是可数集

证明：由命题3可知$$ Z $$是可数集，$$ Z\setminus \{0\} $$也是可数集，则令$$ f(a,b)=a/b $$，由命题2的推论可知$$ \mathbb{Q} $$是可数集。
...
引理的证明（巧妙）：令$$ S_0=X $$,$$ a_n\in S_{n-1} $$,$$ S_n=S_{n-1}\setminus\{a_n\} $$。因为$$ X $$是无限集，因此不存在$$ S_n=\emptyset $$，因此根据选择公理，存在无穷数列$$ a_n $$，$$ a_n $$构成双射，$$ \{a_n:n\in \mathbb{N}\} $$是可数子集。
...
证明：如果$$ I $$是有限集，对$$ \#(I) $$归纳利用可数性命题3得证。对于可数集$$ I $$，证明的思路是转化为$$ \mathbb{N}\times \mathbb{N} $$。存在$$ f:\mathbb{N}\to I $$，$$ \bigcup_{\alpha\in I}A_{\alpha}=\bigcup_{i=0}^{\infty}A_{f(i)}=\bigcup_{i=0}^{\infty}(A_{f(i)}\setminus \bigcup_{j=0}^{i-1}A_{f(j)}) $$，因此它的基数等于$$ \bigcup_{i=0}^{\infty}\big( (A_{f(i)}\setminus \bigcup_{j=0}^{i-1} A_{f(j)})\times \{i\} \big) $$的基数，而$$ \bigcup_{i=0}^{\infty}\big( (A_{f(i)}\setminus \bigcup_{j=0}^{i-1}A_{f(j)})\times \{i\} \big)\subseteq \bigcup_{i=0}^{\infty} \big( A_{f(i)}\times \{i\} \big) $$，对于每个$$ A_{f(i)} $$，都存在一个满射$$ g_i:\mathbb{N}\to A_{f(i)} $$，$$ g_i\in \{h\in (A_{f(i)})^\mathbb{N}:h是满射\} $$，根据选择公理，对每个$$ i\in \mathbb{N} $$存在函数$$ g_i $$，$$ g_i(\mathbb{N})=A_{f(i)} $$，则$$ \bigcup_{i=0}^{\infty} \big(A_{f(i)}\times \{i\}\big)=\{(g_n(m),n):(n,m)\in \mathbb{N}\times \mathbb{N}\} $$，由可数性命题4(至多可数集的笛卡尔积是至多可数集)和可数性命题2(至多可数集经过替代公理仍然得到至多可数集)可知，$$ \{(g_n(m),n):(n,m)\in \mathbb{N}\times \mathbb{N}\} $$至多可数，再由可数性命题1(至多可数集的子集是至多可数集)可知，最终结果至多可数。

## 在无限集上求和

- **可数集上的级数的定义**：设$$ X $$是可数集，$$ f:X\to \mathbb{R} $$是一个函数，称$$ \sum_{x\in X}f(x) $$是绝对收敛的，当且仅当存在某个双射$$ g:\mathbb{N}\to X $$使得$$ \sum_{n=0}^{\infty}f(g(n)) $$是绝对收敛的，此时$$ \sum_{x\in X}f(x):=\sum_{n=0}^{\infty}f(g(n)) $$。

根据重排列命题1，不同的$$ g $$得到的值相同。

- **无限和的富比尼定理**：设$$ f:\mathbb{N}\times \mathbb{N}\to \mathbb{R} $$是使得$$ \sum_{(n,m)\in \mathbb{N}\times \mathbb{N}}f(n,m) $$绝对收敛的函数，那么$$ \sum_{n=0}^{\infty}(\sum_{m=0}^{\infty}f(n,m))=\sum_{(n,m)\in \mathbb{N}\times \mathbb{N}}f(n,m)=\sum_{(m,n)\in \mathbb{N}\times \mathbb{N}}f(n,m)=\sum_{m=0}^{\infty}(\sum_{n=0}^{\infty}f(n,m)) $$。

证明：第二个等号，令双射$$ h(x,y)=(y,x) $$，再根据重排列命题1可得证；先考虑$$ f(n,m)\ge 0 $$的情况。第一个等号和第三个等号证法相同：令$$ L=\sum_{(n,m)\in \mathbb{N}\times \mathbb{N}}f(n,m) $$，要证明左式收敛于$$ L $$。

先证明左式小于等于$$ L $$。利用序列上下确界命题1(无穷递增数列收敛于最小上界)，设双射$$ h:\mathbb{N}\to \mathbb{N}\times \mathbb{N} $$，$$ L $$也是上界，那么对于任何一个有限集$$ X\subseteq \mathbb{N}\times \mathbb{N} $$，存在$$ M $$满足对所有$$ (n,m)\in X $$有$$ h^{-1}(n,m)\le M $$，$$ \sum_{(n,m)\in X}f(n,m)\le \sum_{i\le M}f(h(i))\le L $$。利用有限级数的习题(有限级数和求极限可交换次序)和无限极数的定义，$$ \sum_{n=0}^{N}\sum_{m=0}^{\infty}f(n,m)=\sum_{m=0}^{\infty}\sum_{n=0}^{N}f(n,m) $$，对于任意$$ M $$，根据有限级数的富比尼定理可知，$$ \sum_{m=0}^{M}\sum_{n=0}^{N}f(n,m)=\sum_{(n,m)\in X}f(n,m)\le L $$，再根据最小上界的定义，$$ M\to \infty $$时的无限极数收敛，$$ \sum_{n=0}^{N}\sum_{m=0}^{\infty}f(n,m)\le L $$，再次利用最小上界的定义可知，$$ N\to \infty $$时的无限极数也收敛，即$$ \sum_{n=0}^{\infty}\sum_{m=0}^{\infty}f(n,m)\le L $$。

再证明对于任意$$ \varepsilon\gt 0 $$，有左式大于$$ L-\varepsilon $$。这里的证明方法类似于重排列命题1，将集合看作前缀的子集直接放缩为前缀。根据最小上界的定义可知，存在有限集$$ X\subseteq \mathbb{N}\times \mathbb{N} $$使得$$ \sum_{(n,m)\in X}f(n,m)\gt L-\varepsilon $$，存在集合$$ Y=\{(n,m)\in \mathbb{N}\times \mathbb{N}:n\le N,m\le M\} $$使得$$ X\subseteq Y $$，那么$$ \sum_{n=0}^{N}\sum_{m=0}^{M}f(n,m)=\sum_{(n,m)\in Y}f(n,m)\ge \sum_{(n,m)\in X}f(n,m)\gt L-\varepsilon $$。进而有$$ \sum_{n=0}^{\infty}\sum_{m=0}^{\infty}f(n,m)\ge \sum_{n=0}^{N}\sum_{m=0}^{\infty}f(n,m)\ge \sum_{n=0}^{N}\sum_{m=0}^{M}f(n,m)\gt L-\varepsilon $$。
...
- **任意集合上级数的定义**：对于集合$$ X $$，设$$ f:X\to \mathbb{R} $$，级数$$ \sum_{x\in X}f(x) $$绝对收敛当且仅当$$\sup\{\sum_{x\in A}\vert f(x)\vert :A\subseteq X,A是有限集\} $$不为$$ \infty $$

若$$ X $$是至多可数的，$$\sup\{\sum_{x\in A}\vert f(x)\vert :A\subseteq X,A是有限集\}=\sum_{x\in A}f(x) $$(将两边都利用定义展开到$$\sup $$和有限级数即可)，因此这个定义是可行的。

- **引理1**：对集合$$ X $$，设$$ f:X\to \mathbb{R} $$，级数$$ \sum_{x\in X}f(x) $$绝对收敛，则集合$$ \{x\in X:f(x)\ne 0\} $$是至多可数的。
...
什么发散的级数$$ \sum_{n=0}^{\infty}a_n $$可以通过重排列得到收敛的级数：$$ A_+ $$和$$ A_- $$的级数必须都发散，且存在排列$$ g:\mathbb{N}\to A_+ $$使得$$ \lim_{n\to \infty}a_{g(n)}=0 $$($$ A_- $$同理)。

证明：必要性设存在$$ f $$使得$$ \sum_{n=0}^{\infty}a_{f(n)} $$收敛，令$$ g(n)=f(min\{m\in \mathbb{N}:a_{f(m)}\gt 0且\forall i\lt n有g(i)\ne m\}) $$可得$$ \lim_{n\to \infty}a_{g(n)}=0 $$。充分性可利用上述定理的证明方式，证明其可通过重排列收敛于任意实数。
...
证明：$$ 2^\mathbb{N} $$与$$ \mathbb{N} $$基数不同，是不可数集或有限集。而$$ \{\{n\}:n\in \mathbb{N}\} $$是可数集，也是$$ 2^\mathbb{N} $$的子集，根据基数算数定理2可知$$ 2^\mathbb{N} $$不是有限集。

为什么不能通过二进制拆分构造$$ \mathbb{N} $$和$$ 2^\mathbb{N} $$的双射？因为$$ 2^\mathbb{N} $$中有无限集，而$$ \mathbb{N} $$中所有对象都是有限的数。$$ 2^\mathbb{N} $$中所有无限集通过二进制求和都无法得到一个自然数。

- **推论**：$$ \mathbb{R} $$是不可数集。

证明：定义$$ f:2^\mathbb{N}\to \mathbb{R} $$，$$ f(A)=\sum_{n\in A}2^{-n} $$，对于任意$$ A\ne B $$，找到$$ (A\setminus B)\cup(B\setminus A) $$中的最小元素即可证明$$ f(A)\ne f(B) $$，因此$$ f $$是单射，$$ f(2^\mathbb{N}) $$与$$ 2^\mathbb{N} $$基数相同，而$$ f(2^\mathbb{N}) $$是$$ \mathbb{R} $$的子集，根据可数性命题1可知，$$ \mathbb{R} $$是不可数集。
...
选择公理并不能由有限选取引理直接得到，例如对于不可数集$$ I $$，我认为我们不知道该怎样构造出$$ x_{\alpha} $$，因为即便我们的数学归纳法可以归纳到无穷大（实际上并不能），我们也不知道该以什么样的顺序来取$$ \alpha $$使得所有$$ x_{\alpha} $$的值都被指定了（因为不存在一个$$ \mathbb{N}\to I $$的双射）。
...
证明：设全序集为$$ Y $$。反证，令$$ S_0=Y $$,$$ a_0 $$为$$ S_0 $$中最小元素，$$ S_n=\{x\in Y:x\gt a_{n-1}\} $$,$$ a_n $$是$$ S_n $$中最小元素。利用无限集可得$$ S_n\ne \emptyset $$，则$$ a $$是无穷单调递增的数列。对于集合$$ A=\{x\in Y:\exists n\in \mathbb{N},a_n=x\} $$，$$ A $$中有最大元素，矛盾。（如果$$ S $$是不可数集，其实$$ a $$不可能取遍$$ Y $$中所有元素，所以只能说$$ A $$是$$ Y $$的子集）

- **良序集的定义**：设$$ X $$是偏序集，$$ Y $$是$$ X $$的一个全序子集，如果$$ Y $$的每个非空子集都有最小元素，则$$ Y $$是良序的。

$$ \mathbb{N} $$是良序集。我自己发现了一个不可数良序集的例子：设$$ 2^\mathbb{N} $$是一个全序集，$$ X\le Y $$定义为$$ X\setminus Y $$的最小元素$$ a $$和$$ Y\setminus X $$的最小元素$$ b $$满足$$ a\le b $$(这里认为空集的最小元素是$$ +\infty $$)。其实就是从小到大写出来的序列$$ X $$的字典序更小。对于任意$$ 2^\mathbb{N} $$的子集$$ S $$，只需要每次选出最小自然数使得存在一个$$ S $$中的集合包含它，然后在$$ S $$中删去不包含它的集合。具体地，令$$ S_0=S $$，$$ a_n=\min\{x\in \mathbb{N},x\gt a_{n-1}:\exists Y\in S_n,x\in Y\} $$，$$ S_{n+1}=\{A\in S_n:a_n\in A\} $$。那么对$$ n $$归纳可得：对于每个$$ n $$，任取一个$$ A\in S_{n+1} $$，任取$$ 0\le m\le n $$，都有$$ a_m\in A $$。且对于任意$$ A\in S_n $$和$$ B\notin S_n $$，都有$$ A\le B $$。如果存在$$ S_{n+1}=\emptyset $$（最小集合是有限集），那么$$ \{a_n:0\le m\le n\} $$即为所求；如果不存在，根据选择公理，$$ \{a_n:n\ge 0\} $$即为所求。
...
可以按照常识定义区间$$ [a,b] $$,$$ (a,b) $$,$$ [a,b) $$,$$ (a,b] $$，其中$$ a,b\in \mathbb{R}^* $$是广义实数。当区间表示空集或单个元素时，称这个区间是退化的。

- **$$ \varepsilon $$-附着点的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ \varepsilon\gt 0 $$，$$ x\in \mathbb{R} $$，称$$ x $$是$$ \varepsilon $$-附着于X的，当且仅当存在$$ y\in X $$是$$ \varepsilon $$-接近于$$ x $$的。
- **附着点的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ x\in \mathbb{R} $$，称$$ x $$是$$ \varepsilon $$-附着于$$ X $$的，当且仅当对任意$$ \varepsilon\gt 0 $$，都有$$ x $$是$$ \varepsilon $$-附着于$$ X $$的。
- **闭包的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ X $$的闭包记做$$ \overline{X} $$，表示$$ X $$的全体附着点构成的集合。
...
- 习题1：设$$ X $$是$$ \mathbb{R} $$的子集，$$ X\subseteq Y\subseteq \overline{X} $$，那么必有$$ \overline{Y}=\overline{X} $$
...
另外也很容易证明对于实数$$ a\lt b $$，$$ [a,b] $$,$$ (a,b) $$,$$ [a,b) $$,$$ (a,b] $$的闭包都是$$ [a,b] $$；$$ \mathbb{Q} $$的闭包是$$ \mathbb{R} $$：由实数的序命题3(对任意实数$$ x\lt y $$，存在有理数$$ q $$满足$$ x\lt q\lt y $$)可知对$$ x\in \mathbb{R} $$均存在有理数$$ q $$满足$$ x-\varepsilon\lt q\lt x+\varepsilon $$。

- **引理**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ x\in \mathbb{R} $$，那么$$ x $$是$$ X $$的附着点当且仅当存在一个序列$$ (a_n)_{n=0}^{\infty} $$满足任意对所有$$ n\ge 0 $$有$$ a_n\in X $$，且收敛于$$ x $$。

证明：如果是附着点，类似选择公理的引理的证明思路（利用$$ 1/n $$收敛于0），设$$ S_n=\{x'\in X:\vert x'-x\vert \le 1/n\} $$，再利用选择公理证明$$ a_n\in S_n $$存在；如果存在序列，利用序列收敛的定义即可。

- **闭集的定义**：如果$$ \overline{E}=E $$，那么$$ E\subseteq \mathbb{R} $$是闭的，即$$ E $$的所有附着点都在$$ E $$中。
...
- **极限点的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，称$$ x $$是$$ X $$的极限点当且仅当$$ x $$是$$ X\setminus \{x\} $$的附着点；称$$ x $$是$$ X $$的孤立点当且仅当$$ x\in X $$且$$ x $$不是$$ X\setminus \{x\} $$的附着点
...
- **海涅-博雷尔定理**：设$$ X $$是$$ \mathbb{R} $$的一个子集，$$ X $$是闭的且是有界的，当且仅当任一个由$$ X $$中元素构成的序列$$ (a_n)_{n=0}^{\infty} $$存在一个子序列收敛于$$ X $$中的某个数$$ L $$。
...
- 习题4：$$ \mathbb{R} $$的任一有限子集都是闭的且是有界的。

证明：利用海涅-博雷尔定理，先通过反证法得到$$ a_n $$中有某个$$ X $$中的元素$$ x $$出现了无穷次，那么$$ x $$的项构成的子序列收敛于$$ x $$。

## 函数的极限值

- **函数在一点处收敛的定义**：设$$ X $$是$$ \mathbb{R} $$的一个子集，$$ f:X\to \mathbb{R} $$，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的附着点，称$$ f $$在点$$ x_0 $$处沿着$$ E $$收敛于$$ L $$，记做$$ \lim_{x\to x_0;x\in E}f(x)=L $$，当且仅当对任意$$ \varepsilon\gt 0 $$，存在一个$$ \delta\gt 0 $$，满足对任意$$ x\in E $$且$$ \vert x-x_0\vert \lt \delta $$，满足$$ \vert f(x)-L\vert \le \varepsilon $$。

书上先定义了$$ \varepsilon $$-接近的和局部$$ \varepsilon $$-接近的，将这些定义全部展开得到上述定义。我认为这样定义是为了加深对定义的理解，如果展开后可以感受到这个定义的实际意义，那么展开定义更加简洁。

当$$ x_0\in E $$时，实际上$$ \lim_{x\to x_0;x\in E}f(x)=f(x_0) $$，$$ x_0\notin E $$时更有意义，我们用$$ x\to x_0;x\in X $$来简化表示$$ x\to x_0;x\in X\setminus\{x_0\} $$，因此这里定义了符号$$ \lim_{x\to x_0} $$，表示的是$$ \lim_{x\to x_0;x\in \mathbb{R}\setminus\{x_0\}} $$，这个符号区别于$$ \lim_{n\to \infty} $$，在命题1中将会看到$$ \lim_{n\to \infty}f(a_n) $$是一个数列的极限，而$$ \lim_{x\to x_0}f(x) $$是无穷个极限相等的数列的极限。

- **命题1**：$$ X $$是$$ \mathbb{R} $$的一个子集，$$ f:X\to \mathbb{R} $$，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的附着点，那么$$ f $$在$$ x_0 $$处沿着$$ E $$收敛于$$ L $$，当且仅当对任意由$$ E $$中的元素构成且收敛于$$ x_0 $$的序列$$ (a_n)_{n=0}^{\infty} $$满足$$ (f(a_n))_{n=0}^{\infty} $$收敛于$$ L $$。

证明：左蕴含右，对任意$$ \varepsilon\gt 0 $$，存在$$ \delta\gt 0 $$，而存在$$ N\ge 0 $$满足对任意$$ n\ge N $$有$$ \vert a_n-x\vert \lt \delta $$，那么就对任意$$ n\ge N $$有$$ \vert f(a_n)-L\vert \le \varepsilon $$；右蕴含左很难直接证，因为很难证明$$ \vert x-x_0\vert \lt \delta $$时都成立，使用反证，存在$$ \varepsilon $$，令$$ S_n=\{x\in \mathbb{R}:\vert x-x_0\vert \lt 1/n且\vert f(x)-L\vert \gt \varepsilon\} $$，利用选择公理得到数列$$ a_n $$。

利用这个命题和实直线子集的引理(存在一个数列收敛于附着点)，再根据序列的极限唯一可得函数在一个附着点处沿着同一集合至多一个极限。

利用这个命题和实直线子集的引理，再根据序列的极限定律，可以得到：

- **定理(函数的极限定律)**：如果$$ X $$是$$ \mathbb{R} $$的一个子集，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的一个附着点，$$ f:X\to \mathbb{R} $$在$$ x_0 $$处沿着$$ E $$有极限$$ L $$，$$ g:X\to \mathbb{R} $$在$$ x_0 $$处沿着$$ E $$有极限$$ M $$，则可以得到函数相加对应在$$ x_0 $$处沿着$$ E $$的极限相加（$$ \lim_{x\to x_0}(f\pm g)(x)=\lim_{x\to x_0}f(x)\pm \lim_{x\to x_0}g(x) $$，相乘，相除，取$$ \max $$和取$$ \min $$都同理。
...
- **命题2**：设$$ X $$是$$ \mathbb{R} $$的一个子集，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的一个附着点，$$ f:X\to \mathbb{R} $$，对任意$$ \delta\gt 0 $$，有$$ \lim_{x\to x_0;x\in E}f(x)=L $$当且仅当$$ \lim_{x\to x_0;x\in E\cap (x_0-\delta,x_0+\delta)}f(x)=L $$。
...
- **无限附着点的定义**：$$ X $$是$$ \mathbb{R} $$的子集，$$ +\infty $$附着于$$ X $$当且仅当对任意$$ M\in \mathbb{R} $$都存在$$ x\in X $$满足$$ x\gt M $$；$$ -\infty $$附着于$$ X $$当且仅当对任意$$ M\in \mathbb{R} $$都存在$$ x\in X $$满足$$ x\lt M $$。
- **无限处极限的定义**：$$ X $$是$$ \mathbb{R} $$的子集且$$ +\infty $$是$$ X $$的附着点，$$ f:X\to \mathbb{R} $$，称当$$ x\to \infty $$时$$ f(x) $$沿着$$ X $$收敛于$$ L $$，记做$$ \lim_{x\to +\infty;x\in X}f(x)=L $$，当且仅当对任意$$ \varepsilon $$，都存在$$ M $$使得对所有$$ x\in X $$且$$ x\gt M $$都有$$ \vert f(x)-L\vert \le \varepsilon $$。当$$ x\to -\infty $$同理。

函数的极限定律在无限处也同样适用。

无限处极限的定义很类似于序列极限的定义，可以注意到，当$$ X $$是自然数集$$ \mathbb{N} $$时，两者相等，即$$ \lim_{n\to +\infty;n\in \mathbb{N}}a_n=\lim_{n\to \infty}a_n $$。

## 连续函数和左右极限

- **连续的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$，设$$ x_0\in X $$，称$$ f $$在$$ x_0 $$处是连续的当且仅当$$ \lim_{x\to x_0;x\in X}f(x)=f(x_0) $$，反之我们称$$ f $$在$$ x_0 $$处是间断的。

利用极限值的命题1也可以得到函数连续的定义等价于：对任意有$$ X $$中元素构成且收敛于$$ x_0 $$的序列$$ (a_n)_{n=0}^{\infty} $$满足$$ \lim_{n\to \infty}f(a_n)=f(x_0) $$。

分别利用函数的极限定律和极限值的命题1易得：

- **定理(算数运算保持连续性)**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$,$$ g:X\to \mathbb{R} $$，设$$ x_0\in X $$，如果$$ f $$和$$ g $$在$$ x_0 $$处都连续那么$$ f\pm g $$,$$ fg $$,$$ \max(f,g) $$都在$$ x_0 $$处连续，如果$$ g $$在$$ X $$上不为$$ 0 $$，则$$ f/g $$也在$$ x_0 $$处连续。
- **定理(复合运算保持连续性)**：设$$ X $$和$$ Y $$是$$ \mathbb{R} $$的子集，$$ f:X\to Y $$,$$ g:Y\to \mathbb{R} $$，设$$ x_0 $$是$$ X $$中的点，如果$$ f $$在$$ x_0 $$处连续，$$ g $$在$$ f(x_0) $$处连续，那么$$ g\circ f:X\to \mathbb{R} $$在$$ x_0 $$处连续。
...
- **左右极限的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$，$$ x_0 $$是$$ X\cap (x_0,+\infty) $$的一个附着点，定义$$ f $$在$$ x_0 $$处的右极限$$ f(x_0+):=\lim_{x\to x_0;x\in X\cup (x_0,+\infty}f(x) $$；类似地若$$ x_0 $$是$$ X\cap (-\infty,x_0) $$的一个附着点，定义左极限$$ f(x_0-):=\lim_{x\to x_0;x\in X\cap (-\infty,x_0)}f(x) $$。

- **命题**：设$$ X $$是包含$$ x_0 $$的子集，并且$$ x_0 $$是$$ X\cap (x_0,+\infty) $$和$$ X\cap (-\infty,x_0) $$的附着点，$$ f:X\to \mathbb{R} $$在$$ x_0 $$处连续当且仅当$$ f(x_0+) $$和$$ f(x_0-) $$都存在且为$$ f(x_0) $$。
...
- **振荡间断点**：$$ f(x_0+) $$或$$ f(x_0-) $$不存在且有界，如$$ f(x)=[x\in \mathbb{Q}] $$的$$ 0 $$处

## 最大值原理

函数$$ f:X\to \mathbb{R} $$是有界的定义为存在一个$$ M $$，满足$$ x\in X $$都有$$ \vert f(x)\vert \le M $$。

- **引理**：设$$ a\lt b $$，设$$ f:[a,b]\to \mathbb{R} $$是$$ [a,b] $$上的连续函数，则$$ f $$是一个有界函数。
...
- **函数最值的定义**：对$$ f:X\to \mathbb{R} $$，设$$ x_0\in X $$，如果对所有$$ x\in X $$都有$$ f(x_0)\ge f(x) $$，则称$$ f $$在$$ x_0 $$处取得最大值；如果对所有$$ x\in X $$都有$$ f(x_0)\le f(x) $$，则称$$ f $$在$$ x_0 $$处取得最小值。

- **最大值原理**：设$$ a\lt b $$，$$ f:[a,b]\to \mathbb{R} $$是$$ [a,b] $$上的连续函数，则$$ f $$在某一点处取得最大值，某一点处取得最小值。
...
- **中值定理**：设$$ a\lt b $$，$$ f:[a,b]\to \mathbb{R} $$是$$ [a,b] $$上的连续函数，设$$ y $$是$$ f(a) $$和$$ f(b) $$之间的一个实数，那么存在$$ c\in [a,b] $$满足$$ f(c)=y $$
...
我们也可以直接得到：设$$ f:[a,b]\to \mathbb{R} $$的最小值是$$ m $$，最大值是$$ M $$，则$$ f([a,b])=[m,M] $$
...
- $$ f:X\to \mathbb{R} $$单调递增定义为对任一$$ a,b\in X $$,$$ a\lt b $$,都有$$ f(a)\lt f(b) $$；严格单调递增定义为都有$$ f(a)\lt f(b) $$。显然对于单调函数$$ a\lt b $$当且仅当$$ f(a)\lt f(b) $$。

对于函数$$ f:[a,b]\to \mathbb{R} $$，如果$$ f $$是连续的单射，一定是单调的：设$$ f(a)\le f(b) $$,如果存在$$ x_1\lt x_2 $$有$$ f(x_1)\gt f(x_2) $$，那么利用中值定理存在$$ x_3\in (a,x_1) $$有$$ f(x_3)=f(x_2) $$。

对于函数$$ f:[a,b]\to \mathbb{R} $$，如果$$ f $$是连续的且严格单调的，那么$$ f:[a,b]\to [f(a),f(b)] $$是双射，$$ f^{-1} $$也是严格单调的，且是连续的：直接用$$ \varepsilon-\delta $$定义，对任一$$ y $$和$$ \varepsilon $$，设$$ f(x)=y $$，必存在$$ \delta $$满足$$ f(x-\varepsilon)\le y-\delta\le f(x+\varepsilon) $$。

- 习题：由于$$ \mathbb{Q} $$是可数集，存在双射$$ q:\mathbb{N}\to \mathbb{Q} $$，定义$$ g:\mathbb{Q}\to \mathbb{R} $$满足$$ g(q(n))=2^{-n} $$，则$$ \sum_{r\in \mathbb{Q}}g(r) $$绝对收敛，令$$ f(x)=\sum_{r\in \mathbb{Q};r\lt x}g(r) $$。

(a)$$ f $$单调递增：根据实数的序命题3(任意两实数间存在有理数)，对任意$$ a\lt b $$有实数$$ c\in (a,b) $$，$$ f(b)=f(a)+\sum_{r\in \mathbb{Q};a\le x\lt b}g(r)\ge f(a)+g(c) $$。

(b)$$ f $$在有理数处间断：对有理数$$ a $$，对任意$$ b\gt a $$有$$ f(b)=f(a)+\sum_{r\in \mathbb{Q};a\le r\lt b}g(r)\ge f(a)+g(a) $$。

(c)$$ f $$在无理数处连续(巧妙！)：令$$ f_n(x)=\sum_{r\in \mathbb{Q};r\lt x;g(r)\ge 2^{-n}}g(r) $$，由于$$ \{r\in \mathbb{Q};g(r)\ge 2^{-n}\} $$是有限集，$$ x $$是无理数时存在一个$$ \delta $$，满足$$ [x-\delta,x+\delta) $$中没有该集合的元素，因此$$ f_n $$在无理数处连续，而$$ f_n(x)\le f(x)=f_n(x)+\sum_{r\in \mathbb{Q};r\lt x;g(r)\lt 2^{-n}}g(r)\le f_n(x)+\sum_{r\in \mathbb{Q}:g(r)\lt 2^{-n}}g(r)=f_n(x)+2^{-n} $$，那么再利用函数连续的定义，存在一个$$ n $$满足$$ 2^{-n+1}\lt \varepsilon $$，由于存在$$ \delta $$满足$$ f_n $$在$$ (x-\delta,x+\delta) $$内与$$ f_n(x) $$差不超过$$ \varepsilon-2^{-n+1} $$，$$ f $$在$$ (x-\delta,x+\delta) $$内与$$ f(x) $$差不超过$$ \varepsilon $$。

## 一致连续性

- **一致连续的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，称$$ f:X\to \mathbb{R} $$是一致连续的，当且仅当对任意$$ \varepsilon\gt 0 $$，都存在$$ \delta\gt 0 $$使得对所有$$ \vert x-x_0\vert \le \delta $$都有$$ \vert f(x)-f(x_0)\vert \le \varepsilon $$。

$$ f(x)=1/x $$和$$ f(x)=x^2 $$都是连续但不一致连续的例子，而$$ f(x)=ax+b $$和$$ f(x)=\sqrt{x} $$在$$ (1,+\infty) $$上都满足一致连续，我认为一致连续差不多叙述的就是“导函数在任一处(包括无穷大处)极限不为无穷”。

- **命题1**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$，则$$ f $$在$$ X $$上一致连续，当且仅当对任意两个由$$ X $$中元素构成的等价序列$$ (x_n)_{n=0}^{\infty} $$和$$ (y_n)_{n=0}^{\infty} $$，序列$$ (f(x_n))_{n=0}^{\infty} $$和$$ (f(y_n))_{n=0}^{\infty} $$也等价。
...
- **命题2**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$是一致连续的，$$ (x_n)_{n=0}^{\infty} $$是由$$ X $$中元素构成的柯西序列，那么$$ (f(x_n))_{n=0}^{\infty} $$也是柯西序列。

推论：如果$$ x_0 $$是$$ X $$的附着点，极限$$ \lim_{x\to x_0;x\in X}f(x) $$存在。根据命题2可知函数值序列收敛，再根据命题1可知他们都收敛于同一个实数。

- **命题3**：设$$ X $$是$$ \mathbb{R} $$的子集，$$ f:X\to \mathbb{R} $$是一致连续的，如果$$ E $$是$$ X $$的有界子集，那么$$ f(E) $$也是有界的。

证明：用类似最大值原理引理的证明思路。反证，先找到$$ a_n $$满足$$ f(a_n)\ge n $$，再根据波尔查诺-魏尔斯特拉定理得到$$ a_n $$存在一个收敛的子序列，再根据命题2得到这个子序列对应的函数值序列收敛，矛盾。

- **命题4**：设$$ a\lt b $$都是实数，$$ f:[a,b]\to \mathbb{R} $$是$$ [a,b] $$上的连续函数，那么$$ f $$也是一致连续的。

证明：反证，用命题1。令$$ E=\{n\in \mathbb{N}:\vert f(x_n)-f(y_n)\vert \gt \varepsilon\} $$，根据可数性命题1(可数集的子集至多可数)$$ E $$是可数集，并且存在单调递增序列$$ n $$满足$$ \vert f(x_{n_j})-f(y_{n_j})\vert \gt \varepsilon $$，再利用海涅-博雷尔定理找到$$ (x_{n_j})_{j=0}^{\infty} $$收敛于$$ L\in [a,b] $$的子序列$$ (x_{n_{j_k}})_{k=0}^{\infty} $$，再利用连续函数的定义得$$ \lim_{k\to \infty}f(x_{n_{j_k}})=f(L) $$，由等价序列的定义可知$$ \lim_{n\to \infty}(x_n-y_n)=0 $$，再利用子序列定理1(收敛序列的子序列收敛)得到$$ \lim_{n\to \infty}(x_{n_{j_k}}-y_{n_{j_k}})=0 $$，再根据极限定律得到$$ \lim_{k\to \infty}y_{n_{j_k}}=L $$，有$$ \lim_{k\to \infty}f(y_{n_{j_k}})=f(L) $$，再利用极限定律得到$$ \lim_{k\to \infty}(f(x_{n_{j_k}})-f(y_{n_{j_k}}))=0 $$，与$$ n $$序列的定义矛盾。

# 第10章 函数的微分

## 基本定义

- **可微性的定义**：$$ x_0\in X $$是$$ X $$的极限点，对$$ f:X\to \mathbb{R} $$，如果极限$$ \lim_{x\to x_0;x\in X\setminus\{x_0\}}\frac{f(x)-f(x_0)}{x-x_0} $$收敛于实数$$ L $$，那么$$ f $$在$$ x_0 $$处可微，导数是$$ L $$，记做$$ f'(x_0)=L $$。

可以直接通过函数的极限定律求一些导数：$$ f(x)=x^2 $$时$$ f'(x)=\lim_{x\to x_0;x\in \mathbb{R}\setminus\{x_0\}}\frac{x^2-{x_0}^2}{x-x_0}=\lim_{x\to x_0}x+x_0=2x_0 $$

- **牛顿逼近法**：$$ x_0\in X $$是$$ X $$的极限点，对$$ f:X\to \mathbb{R} $$，$$ f $$在$$ X $$中的$$ x_0 $$处可微且导数是$$ L $$当且仅当：对任意$$ \varepsilon\gt 0 $$都存在$$ \delta\gt 0 $$使得对所有$$ x\in X $$且$$ \vert x-x_0\vert \le \varepsilon $$，那么$$ \vert f(x)-(f(x_0)+L(x-x_0)\vert \le \varepsilon\vert x-x_0\vert $$
...
$$ f:X\to \mathbb{R} $$，若$$ f $$在所有$$ x_0\in X $$处都可微，则定义$$ f $$在$$ X $$上可微函数。那么可以得到可微函数是连续函数。
...
对于$$ f(x)=x^n(n\in \mathbb{N}) $$的导数，可以直接使用归纳法，也可用函数极限定律和n次方差公式得到$$ f'(x)=nx^{n-1} $$；对于$$ n\lt 0 $$的情况利用定理6可得结论相同。

- **链式法则**：$$ f:X\to Y $$在$$ x_0 $$处可微,$$ g:Y\to \mathbb{R} $$在$$ y_0=f(x_0) $$处可微，那么$$ g\circ f:X\to \mathbb{R} $$在$$ x_0 $$处可微，且$$ (g\circ f)'(x_0)=g'(y_0)f'(x_0) $$。
...
- **局部极值的定义**：$$ f:X\to \mathbb{R} $$,$$ x_0\in X $$，称$$ f $$在$$ x_0 $$处达到局部最大值当且仅当存在$$ \delta\gt 0 $$使得$$ f $$在$$ X\cap (x_0-\delta,x_0+\delta) $$上达到最大值；局部最小值同理。

- **命题1**：实数$$ a\lt b $$，$$ f:(a,b)\to \mathbb{R} $$，如果$$ x_0\in (a,b) $$时$$ f $$在$$ x_0 $$处可微，且达到局部最值，则$$ f'(x_0)=0 $$。
...
- **罗尔定理**：实数$$ a\lt b $$，$$ f:[a,b]\to \mathbb{R} $$是连续函数，并且在$$ (a,b) $$上可微，如果$$ f(a)=f(b) $$则存在$$ x\in (a,b) $$使得$$ f'(x)=0 $$。

根据最大值原理和命题1可以直接得到结论。需要注意如果最大值和最小值都在端点处那么该函数是常值函数。

- **推论(平均值定理)**：设$$ f:[a,b]\to \mathbb{R} $$是连续函数且在$$ (a,b) $$上可微，存在$$ x\in (a,b) $$满足$$ f'(x)=\frac{f(b)-f(a)}{b-a} $$。

证明：令$$ g(x)=f(x)-\frac{f(b)-f(a)}{b-a}x $$，$$ g(a)=g(b)=0 $$，再利用罗尔定理即可。

- 习题：如果$$ f:\mathbb{R}\to \mathbb{R} $$是可微函数，且$$ f' $$有界，则$$ f $$一致连续。
...
- **命题2**：若函数$$ f:X\to \mathbb{R} $$单调递增且在$$ x_0\in X $$处可微，则$$ f'(x_0)\ge 0 $$；如果单调递减且在$$ x_0 $$处可微，则$$ f'(x_0)\le 0 $$。

反证并用类似命题1的证明方法。

- **命题3**：实数$$ a\lt b $$，$$ f:[a,b]\to \mathbb{R} $$是可微函数，如果对所有$$ x\in [a,b] $$都有$$ f'(x)\gt 0 $$则$$ f $$严格单调递增；如果都有$$ f'(x)\lt 0 $$则$$ f $$严格单调递减。
...
- 习题：研究函数$$ f(x)=x^a $$，$$ a=1/n(n\in \mathbb{N}且n\ge 1) $$、$$ a\in \mathbb{Q} $$、$$ a\in \mathbb{R} $$

1.$$ a=1/n $$时，根据单调函数的命题(严格单调且连续的函数反函数也连续)，$$ f(x) $$时连续函数，再根据反函数定理得到$$ f'(x)=1/nx^{1/n-1} $$。

2.$$ a\in \mathbb{Q} $$时，令$$ a=p/q $$，$$ x^a=(x^p)^{1/q} $$，使用链式法则和微分计算定理可得$$ f'(x)=ax^{a-1} $$。
...
- **洛必达法则2**：实数$$ a\lt b $$，$$ f:[a,b]\to \mathbb{R} $$和$$ g:[a,b]\to \mathbb{R} $$是$$ [a,b] $$上的可微函数，如果$$ f(a)=g(a)=0 $$，且$$ g' $$在$$ [a,b] $$上不为零，且极限$$ \lim_{x\to a;x\in (a,b]}\frac{f'(x)}{g'(x)} $$存在且等于$$ L $$，那么对所有$$ x\in (a,b] $$都有$$ g(x)\ne 0 $$，且极限$$ \lim_{x\to a;x\in (a,b]}\frac{f(x)}{g(x)}=L $$。

证明（巧妙！）：对于$$ g(x)\ne 0 $$可反证并用罗尔定理得到矛盾。利用函数极限的命题1转化为序列$$ (x_n)_{n=1}^{\infty} $$收敛于$$ a $$，构造函数$$ h_n(x)=f(x)g(x_n)-g(x)f(x_n):[a,x_n]\to \mathbb{R} $$，根据算数运算保持连续性定理$$ h_n(x) $$连续，且$$ h_n(a)=h_n(x_n)=0 $$，因此根据罗尔定理存在$$ a\lt y_n\lt x_n $$满足$$ h_n'(y_n)=0 $$，根据夹逼定理得到$$ (y_n)_{n=1}^{\infty} $$也收敛于$$ a $$，即$$ \frac{f(x_n)}{g(x_n)}=\frac{f'(y_n)}{g'(y_n)} $$，因此$$ \lim_{n\to \infty}\frac{f(x_n)}{g(x_n)}=\lim_{n\to \infty}{f'(y_n)}{g'(y_n)}=L $$。
...
- **连通的定义**：设$$ X $$是$$ \mathbb{R} $$的子集，称$$ X $$是连通的，当且仅当对任意$$ X $$中的元素$$ x\lt y $$，$$ [x,y] $$是$$ X $$的子集。
...
常数函数$$ f:X\to \mathbb{R} $$定义为对任意$$ x\in X $$都有$$ f(x)=c $$，$$ c $$称为$$ f $$在$$ X $$上的常数值。

- **分段常数函数的定义**：设$$ I $$是有界区间，$$ f:I\to \mathbb{R} $$是一个函数，$$ P $$是$$ I $$的一个划分，如果对任意$$ J\in P $$，$$ f $$在$$ J $$上是常值的，则称$$ f $$是关于$$ P $$的分段常数函数。称$$ f $$是$$ I $$上的分段常数函数。

若$$ f $$是关于$$ P $$的分段常数函数，那么对于任意比$$ P $$更细的划分$$ P' $$，$$ f $$都是关于$$ P' $$的分段常数函数。

- **引理**：$$ I $$是有界区间，且$$ f:I\to \mathbb{R} $$和$$ g:I\to \mathbb{R} $$都是$$ I $$上的分段常数函数，那么$$ f+g $$,$$ max(f,g) $$,$$ fg $$都是$$ I $$上的分段常数函数，如果$$ g $$在任一处不为$$ 0 $$，则$$ f/g $$也是分段常数函数。

对$$ f $$的划分和$$ g $$的划分进行公共细化得到运算后结果的划分。

- **分段常值积分的定义**：设$$ I $$是有界区间，$$ P $$是$$ I $$的一个划分，$$ f:I\to \mathbb{R} $$是关于$$ P $$的分段常数函数，定义$$ f $$的分段常数积分$$ p.c.\int_{I}f:=\sum_{J\in P}c_J\vert J\vert $$，其中对任意$$ J\in P $$，$$ c_J $$表示$$ f $$在$$ J $$上的常数值。

- **引理**：对于$$ I $$的不同划分$$ P $$和$$ P' $$，$$ p.c.\int_{I}f $$通过两个划分得到的结果相同。

证明可以考虑$$ P\#P' $$，对粗糙的划分的基数归纳可得$$ P $$和$$ P' $$均与$$ P\#P' $$得到的结果相同。

设$$ I $$是有界区间，$$ f:I\to \mathbb{R} $$和$$ g:I\to \mathbb{R} $$是$$ I $$上的分段常数函数，有如下积分定律：
...
- **积分定律5**：如果$$ {J,K} $$是$$ I $$的一个划分，那么$$ f\vert _{J}:J\to \mathbb{R} $$和$$ f\vert _{K}:K\to \mathbb{R} $$分别是$$ J $$和$$ K $$上的分段常数函数，且$$ p.c.\int_{I}f=p.c.\int_{J}f\vert _J+p.c.\int_{K}f\vert _K $$。

积分定律1仍然考虑公共细化，积分定律5先证明若两个有界区间$$ I\subseteq J $$,$$ x\in I $$时$$ F(x)=f(x) $$,$$ x\notin I $$时$$ F(x)=0 $$，若$$ f $$是分段常数函数，$$ F $$也是分段常数函数。

## 上黎曼积分和下黎曼积分

- **函数控制的定义**：$$ f:I\to \mathbb{R} $$,$$ g:I\to \mathbb{R} $$，如果对所有$$ x\in I $$都有$$ g(x)\ge f(x) $$，称$$ g $$在$$ I $$上从上方控制$$ f $$；如果对所有$$ x\in I $$都有$$ g(x)\le f(x) $$，称$$ g $$在$$ I $$上从下方控制$$ f $$。

- **上下黎曼积分的定义**：设$$ f:I\to \mathbb{R} $$是定义在有界区间$$ I $$上的函数，对$$ f: $$定义上黎曼积分$$ \overline{\int_{I}}f:=\inf\{p.c.\int_{I}g:g是从上方控制f的分段常数函数\} $$,下黎曼积分$$ \underline{\int_{I}}f:=\sup\{p.c.\int_{I}g:g是从下方控制f的分段常数函数\} $$。

- **引理**：设$$ f:I\to \mathbb{R} $$是定义在有界区间$$ I $$上的有界函数，以$$ M $$为界，则$$ -M\vert I\vert \le \underline{\int_{I}}f \le \overline{\int_{I}}f\le M\vert I\vert $$。

利用积分定律3，再两次运用上下确界的定义即可。

- **黎曼积分的定义**：设$$ f:I\to \mathbb{R} $$是定义在有界区间$$ I $$上的有界函数，如果$$ \underline{\int_{I}}f=\overline{\int_{I}}f $$，则称$$ f $$在$$ I $$上黎曼可积，并定义$$ \int_{I}f:=\underline{\int_{I}}f=\overline{\int_{I}}f $$。

对于分段常数函数$$ f $$，有$$ \int_{I}f=p.c.\int_{I}f $$。

- **黎曼和的定义**：设$$ f:I\to \mathbb{R} $$是定义在有界区间$$ I $$上的有界函数，$$ P $$是$$ I $$的一个划分，定义上黎曼和$$ U(f,P)=\sum_{J\in P;J\ne \emptyset}(\sup_{x\in J}f(x))\vert J\vert $$，下黎曼和$$ L(f,P)=\sum_{J\in P;J\ne \emptyset}(\inf_{x\in J}f(x))\vert J\vert $$。

- **命题**：设$$ f:I\to \mathbb{R} $$是定义在有界区间$$ I $$上的有界函数，则$$ \overline{\int{I}}f=\inf\{U(f,P):P是I的划分\} $$，$$ \underline{\int{I}}f=\sup\{L(f,P):P是I的划分\} $$。
...
- **定理1**：设$$ I $$是有界区间，$$ f:I\to \mathbb{R} $$和$$ g:I\to \mathbb{R} $$是黎曼可积的，则$$ \max(f,g)(x):I\to \mathbb{R} $$和$$ \min(f,g)(x):I\to \mathbb{R} $$也是黎曼可积的。

证明：思路是利用定义并取$$ \varepsilon $$证明两数相等。任取$$ \varepsilon $$，存在从下方控制$$ f $$的分段常数函数$$ \underline{f}:I\to \mathbb{R} $$满足$$ \int_{I}\underline{f}\ge \int_{I}f-\varepsilon $$，从上方控制$$ f $$的分段常数函数$$ \overline{f} $$满足$$ \int_{I}\overline{f}\le \int_{I}f+\varepsilon $$；$$ \underline{g} $$满足$$ \int_{I}\underline{g}\ge \int_{I}f-\varepsilon $$，$$ \overline{g} $$满足$$ \int_{I}\overline{g}\le \int_{I}g+\varepsilon $$。$$ \overline{\int_{I}}\max(f,g)-\underline{\int_{I}}\max(f,g)\le \int_{I}\max(\overline{f},\overline{g})-\int_{I}\max(\underline{f},\underline{g}) $$，而$$ \max(\overline{f},\overline{g})-\max(\underline{f},\underline{g})\le (\overline{f}-\underline{f})+(\overline{g}-\underline{g}) $$，因此$$ \max(f,g)(x) $$的上下黎曼积分差不超过$$ 4\varepsilon $$。

而$$ \vert f\vert =\max(f,-f) $$，绝对值保持黎曼可积性。也可设$$ f_+=max(f,0) $$,$$ f_-=min(f,0) $$,$$ \vert f\vert =f_+-f_- $$。

- **定理2**：设$$ I $$是有界区间，$$ f:I\to \mathbb{R} $$和$$ g:I\to \mathbb{R} $$是黎曼可积的，则$$ (fg)(x):I\to \mathbb{R} $$也是黎曼可积的。
...
- **命题**：设$$ f[0,1]\to \mathbb{R} $$在$$ x\in \mathbb{Q} $$处为$$ 1 $$，$$ x\notin \mathbb{Q} $$处为0，$$ f $$有界但不黎曼可积。
...
- **微积分第一基本定理**：设$$ f:[a,b]\to \mathbb{R} $$是黎曼可积的函数，$$ F:[a,b]\to \mathbb{R} $$是函数$$ \int_{[a,x]}f $$，则$$ F $$是连续的。如果$$ x_0\in [a,b] $$且$$ f $$在$$ x_0 $$处连续，则$$ F $$在$$ x_0 $$处可微，且$$ F'(x_0)=f(x_0) $$。
...
- 习题2：设$$ f:[a,b]\to \mathbb{R} $$时黎曼可积的函数，且$$ f $$单调，求证如果$$ x_0\in [a,b] $$，则$$ f $$在$$ x_0 $$处连续当且仅当$$ F $$在$$ x_0 $$处可微。
...
- **原函数的定义**：设$$ I $$是有界区间，$$ f:I\to \mathbb{R} $$，函数$$ F:I\to \mathbb{R} $$可微，且对所有$$ x\in I $$都有$$ F'(x)=f(x) $$，则称$$ F $$是$$ f $$的原函数。

我认为微积分第一基本定理的内容简单来说就是若$$ F(b)=\int_{[a,b]}f $$，则$$ F $$是$$ f $$的原函数，而微积分第二基本定理的内容就是若$$ F $$是$$ f $$的原函数，则$$ F(b)-F(a)=\int_{[a,b]}f $$。几乎是互为逆命题的。

- **微积分第二基本定理**：设$$ a\lt b $$是实数，设$$ f:[a,b]\to \mathbb{R} $$是黎曼可积的函数，如果$$ F:[a,b]\to \mathbb{R} $$是$$ f $$的原函数，那么$$ \int_{[a,b]}f=F(b)-F(a) $$。
...
- **引理**：设$$ I $$是有界区间，对$$ f:I\to \mathbb{R} $$，设$$ F:I\to \mathbb{R} $$和$$ G:I\to \mathbb{R} $$是$$ f $$的两个原函数，存在实数$$ C $$使得对所有$$ F=G+C $$
