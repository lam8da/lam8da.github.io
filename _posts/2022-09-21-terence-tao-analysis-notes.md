...
可直接构造证明：设$$ f:X\to \{i\in N:1\le i\le n\} $$，令$$ f(x')\lt f(x) $$时$$ g(x')=f(x') $$，$$ f(x')\gt f(x) $$时$$ g(x')=f(x')-1 $$即可，再证明是双射即可。
...
注意并不能说$$ E $$中最大元素是上界，$$ E $$是无限集，且很可能不存在最大值如$$ {x\in R:1\lt x\lt 2} $$
...
证明（巧妙！）：思路是利用上确界。令$$ E={y\in R:y\ge 0且y^2\lt 2} $$，显然$$ 2 $$是$$ E $$的上界，因此存在$$ x=\sup(E) $$，利用实数的三歧性。
...
定理2的左蕴含右证明用类似实数的上界存在性的证法（因为是先构造序列，再证明对所有$$ \varepsilon $$成立，所以利用$$ \lim_{n\to \infty}1/n=0 $$）。根据$$ L $$为极限点，令$$ \varepsilon=1/n $$可得集合$$ \{m\in N:m\gt f(n-1)且d(a_m,L)\le 1/n\} $$不为空，令$$ f(n) $$为集合中任一元素即可。
...
证明：思路是找到集合$$ X\subseteq N $$且$$ f:X\to f(N) $$构成双射，再利用命题1得到$$ X $$至多可数。令$$ X=\{x\in N:\forall x'\lt x有f(x)\ne f(x')\} $$即可。
...
证明：思路是按照一定顺序“分配”给自然数$$ (1,1),(2,1),(2,2),(3,1),(3,2),(3,3),... $$。令$$ a_0=0 $$,$$ a_{n+1}=a_n+n+1 $$，那么$$ a_N $$就表示$$ n\lt N $$的二元组个数，直接令$$ f(n,m):=a_n+m $$即可。只需说明$$ f $$是一对一的即可说明存在$$ A $$到$$ f(A) $$的双射，由命题1的推论1可知其至多可数，而如果$$ A $$有限，则任一个子集都有限，而$$ A'=\{(n,0)\in N\times N:n\in N\} $$与$$ N $$基数相同，矛盾，因此$$ A $$是可数的。（直接说明$$ f $$是满射似乎更方便）
...
再证明对于任意$$ \varepsilon\gt 0 $$，有左式大于$$ L-\varepsilon $$。这里的证明方法类似于重排列命题1，将集合看作前缀的子集直接放缩为前缀。根据最小上界的定义可知，存在有限集$$ X\subseteq N\times N $$使得$$ \sum_{(n,m)\in X}f(n,m)\gt L-\varepsilon $$，存在集合$$ Y=\{(n,m)\in N\times N:n\le N,m\le M\} $$使得$$ X\subseteq Y $$，那么$$ \sum_{n=0}^{N}\sum_{m=0}^{M}f(n,m)=\sum_{(n,m)\in Y}f(n,m)\ge \sum_{(n,m)\in X}f(n,m)\gt L-\varepsilon $$。进而有$$ \sum_{n=0}^{\infty}\sum_{m=0}^{\infty}f(n,m)\ge \sum_{n=0}^{N}\sum_{m=0}^{\infty}f(n,m)\ge \sum_{n=0}^{N}\sum_{m=0}^{M}f(n,m)\gt L-\varepsilon $$。
...
- **引理2**：若$$ \sum_{n=0}^{\infty}a_n $$收敛但不绝对收敛，则令$$ A_+=\{n\in N:a_n\ge 0\} $$,$$ A_-=\{n\in N:a_n\lt 0\} $$，则级数$$ \sum_{n\in A_+}a_n $$和$$ \sum_{n\in A_-}a_n $$都不收敛。
...
证明：必要性设存在$$ f $$使得$$ \sum_{n=0}^{\infty}a_{f(n)} $$收敛，令$$ g(n)=f(min\{m\in N:a_{f(m)}\gt 0且\forall i\lt n有g(i)\ne m\}) $$可得$$ \lim_{n\to \infty}a_{g(n)}=0 $$。充分性可利用上述定理的证明方式，证明其可通过重排列收敛于任意实数。
...
$$ N $$是良序集。我自己发现了一个不可数良序集的例子：设$$ 2^N $$是一个全序集，$$ X\le Y $$定义为$$ X\setminus Y $$的最小元素$$ a $$和$$ Y\setminus X $$的最小元素$$ b $$满足$$ a\le b $$(这里认为空集的最小元素是$$ +\infty $$)。其实就是从小到大写出来的序列$$ X $$的字典序更小。对于任意$$ 2^N $$的子集$$ S $$，只需要每次选出最小自然数使得存在一个$$ S $$中的集合包含它，然后在$$ S $$中删去不包含它的集合。具体地，令$$ S_0=S $$，$$ a_n=\min\{x\in N,x\gt a_{n-1}:\exists Y\in S_n,x\in Y\} $$，$$ S_{n+1}=\{A\in S_n:a_n\in A\} $$。那么对$$ n $$归纳可得：对于每个$$ n $$，任取一个$$ A\in S_{n+1} $$，任取$$ 0\le m\le n $$，都有$$ a_m\in A $$。且对于任意$$ A\in S_n $$和$$ B\notin S_n $$，都有$$ A\le B $$。如果存在$$ S_{n+1}=\emptyset $$（最小集合是有限集），那么$$ \{a_n:0\le m\le n\} $$即为所求；如果不存在，根据选择公理，$$ \{a_n:n\ge 0\} $$即为所求。
...
- **$$ \varepsilon $$-附着点的定义**：设$$ X $$是$$ R $$的子集，$$ \varepsilon\gt 0 $$，$$ x\in R $$，称$$ x $$是$$ \varepsilon $$-附着于X的，当且仅当存在$$ y\in X $$是$$ \varepsilon $$-接近于$$ x $$的。
- **附着点的定义**：设$$ X $$是$$ R $$的子集，$$ x\in R $$，称$$ x $$是$$ \varepsilon $$-附着于$$ X $$的，当且仅当对任意$$ \varepsilon\gt 0 $$，都有$$ x $$是$$ \varepsilon $$-附着于$$ X $$的。
...
另外也很容易证明对于实数$$ a\lt b $$，$$ [a,b] $$,$$ (a,b) $$,$$ [a,b) $$,$$ (a,b] $$的闭包都是$$ [a,b] $$；$$ Q $$的闭包是$$ R $$：由实数的序命题3(对任意实数$$ x\lt y $$，存在有理数$$ q $$满足$$ x\lt q\lt y $$)可知对$$ x\in R $$均存在有理数$$ q $$满足$$ x-\varepsilon\lt q\lt x+\varepsilon $$。
...
- **函数在一点处收敛的定义**：设$$ X $$是$$ R $$的一个子集，$$ f:X\to R $$，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的附着点，称$$ f $$在点$$ x_0 $$处沿着$$ E $$收敛于$$ L $$，记做$$ \lim_{x\to x_0;x\in E}f(x)=L $$，当且仅当对任意$$ \varepsilon\gt 0 $$，存在一个$$ \delta\gt 0 $$，满足对任意$$ x\in E $$且$$ \vert x-x_0\vert \lt \delta $$，满足$$ \vert f(x)-L\vert \le \varepsilon $$。
...
证明：左蕴含右，对任意$$ \varepsilon\gt 0 $$，存在$$ \delta\gt 0 $$，而存在$$ N\ge 0 $$满足对任意$$ n\ge N $$有$$ \vert a_n-x\vert \lt \delta $$，那么就对任意$$ n\ge N $$有$$ \vert f(a_n)-L\vert \le \varepsilon $$；右蕴含左很难直接证，因为很难证明$$ \vert x-x_0\vert \lt \delta $$时都成立，使用反证，存在$$ \varepsilon $$，令$$ S_n=\{x\in R:\vert x-x_0\vert \lt 1/n且\vert f(x)-L\vert \gt \varepsilon\} $$，利用选择公理得到数列$$ a_n $$。
...
- **命题2**：设$$ X $$是$$ R $$的一个子集，$$ E $$是$$ X $$的一个子集，$$ x_0 $$是$$ E $$的一个附着点，$$ f:X\to R $$，对任意$$ \delta\gt 0 $$，有$$ \lim_{x\to x_0;x\in E}f(x)=L $$当且仅当$$ \lim_{x\to x_0;x\in E\cap (x_0-\delta,x_0+\delta)}f(x)=L $$。
...
- **无限附着点的定义**：$$ X $$是$$ R $$的子集，$$ +\infty $$附着于$$ X $$当且仅当对任意$$ M\in R $$都存在$$ x\in X $$满足$$ x\gt M $$；$$ -\infty $$附着于$$ X $$当且仅当对任意$$ M\in R $$都存在$$ x\in X $$满足$$ x\lt M $$。
- **无限处极限的定义**：$$ X $$是$$ R $$的子集且$$ +\infty $$是$$ X $$的附着点，$$ f:X\to R $$，称当$$ x\to \infty $$时$$ f(x) $$沿着$$ X $$收敛于$$ L $$，记做$$ \lim_{x\to +\infty;x\in X}f(x)=L $$，当且仅当对任意$$ \varepsilon $$，都存在$$ M $$使得对所有$$ x\in X $$且$$ x\gt M $$都有$$ \vert f(x)-L\vert \le \varepsilon $$。当$$ x\to -\infty $$同理。
...
- **引理**：设$$ a\lt b $$，设$$ f:[a,b]\to R $$是$$ [a,b] $$上的连续函数，则$$ f $$是一个有界函数。
...
- **最大值原理**：设$$ a\lt b $$，$$ f:[a,b]\to R $$是$$ [a,b] $$上的连续函数，则$$ f $$在某一点处取得最大值，某一点处取得最小值。
...
- **中值定理**：设$$ a\lt b $$，$$ f:[a,b]\to R $$是$$ [a,b] $$上的连续函数，设$$ y $$是$$ f(a) $$和$$ f(b) $$之间的一个实数，那么存在$$ c\in [a,b] $$满足$$ f(c)=y $$
...
- $$ f:X\to R $$单调递增定义为对任一$$ a,b\in X $$,$$ a\lt b $$,都有$$ f(a)\lt f(b) $$；严格单调递增定义为都有$$ f(a)\lt f(b) $$。显然对于单调函数$$ a\lt b $$当且仅当$$ f(a)\lt f(b) $$。

对于函数$$ f:[a,b]\to R $$，如果$$ f $$是连续的单射，一定是单调的：设$$ f(a)\le f(b) $$,如果存在$$ x_1\lt x_2 $$有$$ f(x_1)\gt f(x_2) $$，那么利用中值定理存在$$ x_3\in (a,x_1) $$有$$ f(x_3)=f(x_2) $$。
...
- 习题：由于$$ Q $$是可数集，存在双射$$ q:N\to Q $$，定义$$ g:Q\to R $$满足$$ g(q(n))=2^{-n} $$，则$$ \sum_{r\in Q}g(r) $$绝对收敛，令$$ f(x)=\sum_{r\in Q;r\lt x}g(r) $$。

(a)$$ f $$单调递增：根据实数的序命题3(任意两实数间存在有理数)，对任意$$ a\lt b $$有实数$$ c\in (a,b) $$，$$ f(b)=f(a)+\sum_{r\in Q;a\le x\lt b}g(r)\ge f(a)+g(c) $$。

(b)$$ f $$在有理数处间断：对有理数$$ a $$，对任意$$ b\gt a $$有$$ f(b)=f(a)+\sum_{r\in Q;a\le r\lt b}g(r)\ge f(a)+g(a) $$。

(c)$$ f $$在无理数处连续(巧妙！)：令$$ f_n(x)=\sum_{r\in Q;r\lt x;g(r)\ge 2^{-n}}g(r) $$，由于$$ \{r\in Q;g(r)\ge 2^{-n}\} $$是有限集，$$ x $$是无理数时存在一个$$ \delta $$，满足$$ [x-\delta,x+\delta) $$中没有该集合的元素，因此$$ f_n $$在无理数处连续，而$$ f_n(x)\le f(x)=f_n(x)+\sum_{r\in Q;r\lt x;g(r)\lt 2^{-n}}g(r)\le f_n(x)+\sum_{r\in Q:g(r)\lt 2^{-n}}g(r)=f_n(x)+2^{-n} $$，那么再利用函数连续的定义，存在一个$$ n $$满足$$ 2^{-n+1}\lt \varepsilon $$，由于存在$$ \delta $$满足$$ f_n $$在$$ (x-\delta,x+\delta) $$内与$$ f_n(x) $$差不超过$$ \varepsilon-2^{-n+1} $$，$$ f $$在$$ (x-\delta,x+\delta) $$内与$$ f(x) $$差不超过$$ \varepsilon $$。

## 一致连续性

- **一致连续的定义**：设$$ X $$是$$ R $$的子集，称$$ f:X\to R $$是一致连续的，当且仅当对任意$$ \varepsilon\gt 0 $$，都存在$$ \delta\gt 0 $$使得对所有$$ \vert x-x_0\vert \le \delta $$都有$$ \vert f(x)-f(x_0)\vert \le \varepsilon $$。
...
- **命题4**：设$$ a\lt b $$都是实数，$$ f:[a,b]\to R $$是$$ [a,b] $$上的连续函数，那么$$ f $$也是一致连续的。
...
- **牛顿逼近法**：$$ x_0\in X $$是$$ X $$的极限点，对$$ f:X\to R $$，$$ f $$在$$ X $$中的$$ x_0 $$处可微且导数是$$ L $$当且仅当：对任意$$ \varepsilon\gt 0 $$都存在$$ \delta\gt 0 $$使得对所有$$ x\in X $$且$$ \vert x-x_0\vert \le \varepsilon $$，那么$$ \vert f(x)-(f(x_0)+L(x-x_0)\vert \le \varepsilon\vert x-x_0\vert $$
...
对于$$ f(x)=x^n(n\in N) $$的导数，可以直接使用归纳法，也可用函数极限定律和n次方差公式得到$$ f'(x)=nx^{n-1} $$；对于$$ n\lt 0 $$的情况利用定理6可得结论相同。
...
- **局部极值的定义**：$$ f:X\to R $$,$$ x_0\in X $$，称$$ f $$在$$ x_0 $$处达到局部最大值当且仅当存在$$ \delta\gt 0 $$使得$$ f $$在$$ X\cap (x_0-\delta,x_0+\delta) $$上达到最大值；局部最小值同理。

- **命题1**：实数$$ a\lt b $$，$$ f:(a,b)\to R $$，如果$$ x_0\in (a,b) $$时$$ f $$在$$ x_0 $$处可微，且达到局部最值，则$$ f'(x_0)=0 $$。
...
- **罗尔定理**：实数$$ a\lt b $$，$$ f:[a,b]\to R $$是连续函数，并且在$$ (a,b) $$上可微，如果$$ f(a)=f(b) $$则存在$$ x\in (a,b) $$使得$$ f'(x)=0 $$。
...
- **命题3**：实数$$ a\lt b $$，$$ f:[a,b]\to R $$是可微函数，如果对所有$$ x\in [a,b] $$都有$$ f'(x)\gt 0 $$则$$ f $$严格单调递增；如果都有$$ f'(x)\lt 0 $$则$$ f $$严格单调递减。
...
- **洛必达法则2**：实数$$ a\lt b $$，$$ f:[a,b]\to R $$和$$ g:[a,b]\to R $$是$$ [a,b] $$上的可微函数，如果$$ f(a)=g(a)=0 $$，且$$ g' $$在$$ [a,b] $$上不为零，且极限$$ \lim_{x\to a;x\in (a,b]}\frac{f'(x)}{g'(x)} $$存在且等于$$ L $$，那么对所有$$ x\in (a,b] $$都有$$ g(x)\ne 0 $$，且极限$$ \lim_{x\to a;x\in (a,b]}\frac{f(x)}{g(x)}=L $$。

证明（巧妙！）：对于$$ g(x)\ne 0 $$可反证并用罗尔定理得到矛盾。利用函数极限的命题1转化为序列$$ (x_n)_{n=1}^{\infty} $$收敛于$$ a $$，构造函数$$ h_n(x)=f(x)g(x_n)-g(x)f(x_n):[a,x_n]\to R $$，根据算数运算保持连续性定理$$ h_n(x) $$连续，且$$ h_n(a)=h_n(x_n)=0 $$，因此根据罗尔定理存在$$ a\lt y_n\lt x_n $$满足$$ h_n'(y_n)=0 $$，根据夹逼定理得到$$ (y_n)_{n=1}^{\infty} $$也收敛于$$ a $$，即$$ \frac{f(x_n)}{g(x_n)}=\frac{f'(y_n)}{g'(y_n)} $$，因此$$ \lim_{n\to \infty}\frac{f(x_n)}{g(x_n)}=\lim_{n\to \infty}{f'(y_n)}{g'(y_n)}=L $$。
...
- **连通的定义**：设$$ X $$是$$ R $$的子集，称$$ X $$是连通的，当且仅当对任意$$ X $$中的元素$$ x\lt y $$，$$ [x,y] $$是$$ X $$的子集。
...
- **积分判别法**：设$$ f:[0,+\infty)\to R $$时单调递减的非负函数，那么级数$$ \sum_{n=0}^{\infty}f(n) $$收敛当且仅当$$ \sup_{N\gt 0}\int_{[0,N]}f $$有限。
...
- **微积分第二基本定理**：设$$ a\lt b $$是实数，设$$ f:[a,b]\to R $$是黎曼可积的函数，如果$$ F:[a,b]\to R $$是$$ f $$的原函数，那么$$ \int_{[a,b]}f=F(b)-F(a) $$。
