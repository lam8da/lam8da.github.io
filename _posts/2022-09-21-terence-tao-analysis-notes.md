...
有序n元组可利用函数定义：对任意集合$$ X $$，满射函数$$ x:\{i\in \NN:1\le i\le n\}\to X $$，令$$ x_i=x(i) $$并用函数相等的定义证明n元组相等的定义。利用这个定义可证明多重笛卡尔积的结果是集合：令$$ X=\bigcup X_i $$，利用幂集公理可知存在集合$$ A=X^{\{i\in \NN:1\le i\le n\}} $$，利用分类公理有$$ \{f\in A:\forall 1\le i\le n有f(i)\in X_i\} $$。
...
- **命题**：自然数系唯一。如果存在另一个满足皮亚诺公理的自然数系$$ \NN' $$，存在双射$$ a:\NN\to \NN' $$满足$$ a(0)=0' $$且$$ \forall n\in \NN,n'\in \NN' $$,$$ a(n)=n' $$当且仅当$$ a(n++)=n'++ $$。

首先证明引理：对$$ f:\NN\times \NN\to \NN $$和自然数$$ c $$，存在函数$$ a:\NN\to \NN $$满足$$ a(0)=c $$且$$ \forall n\in \NN $$有$$ a(n++)=f(n,a(n)) $$。
