...
1.
定义：（达布）上/下和：假设函数某闭区间上有界，则该函数在分割的每个子区间也有界，因此有上确界$$M_i=\sup_{x\in\Delta_i}f(x)$$和下确界$$m_i$$，则$$S(T)=\sum_{i=1}^n M_i\Delta x_i$$称为达布上和（其中$$T$$表示该分割），同理有达布下和。
1. 定理9.3（可积的充要条件）：任给某正数总存在某分割使得对应的上下和的差小于该正数。证明在下面。
