...
- 实向量空间或复向量空间$$V$$上的内积（inner product）就是一个函数，它把$$V$$中元素的每个有序对$$(\vec{u},\vec{v})$$都映成一个数$$\langle\vec{u},\vec{v}\rangle\in\mathbf{F}$$，并且具有性质：
  - 正性：对所有$$\vec{v}\in V$$都有$$\langle\vec{v},\vec{v}\rangle\ge0$$（这意味着$$\langle\vec{v},\vec{v}\rangle$$是一个实数）
  - 定性：$$\langle\vec{v},\vec{v}\rangle=0$$当且仅当$$\vec{v}=0$$
  - 第一个位置的加性：对所有$$\vec{u},\vec{v},\vec{w}\in V$$都有$$\langle\vec{u}+\vec{v},\vec{w}\rangle=\langle\vec{u},\vec{w}\rangle+\langle\vec{v},\vec{w}\rangle$$
    - 通过这个和下一个性质可以证明第二个位置也具有加性，即：$$\langle\vec{u},\vec{v}+\vec{w}\rangle=\langle\vec{u},\vec{v}\rangle+\langle\vec{u},\vec{w}\rangle$$
    - 可证$$\langle\vec{0},\vec{w}\rangle=\langle\vec{w},\vec{0}\rangle=0,\vec{w}\in V$$
  - 第一个位置的齐性：对所有$$a\in\mathbf{F},\vec{v},\vec{w}\in V$$都有$$\langle a\vec{v},\vec{w}\rangle=a\langle\vec{v},\vec{w}\rangle$$
    - 通过这个和上一个性质可以证明第二个位置具有共轭齐性，即：$$\langle\vec{u},a\vec{v}\rangle=\overline{a}\langle\vec{u},\vec{v}\rangle$$
  - 共轭对称性：对所有$$\vec{v},\vec{w}\in V$$都有$$\langle\vec{v},\vec{w}\rangle=\overline{\langle\vec{w},\vec{v}\rangle}$$
    - 对实向量空间这等价于$$\langle\vec{v},\vec{w}\rangle=\langle\vec{w},\vec{v}\rangle$$
- 内积空间是带有内积的向量空间。例子：
...
- 范数定义为$$\| \vec{v}\|=\sqrt{\langle\vec{v},\vec{v}\rangle}$$。性质：
  - $$\| \vec{v}\|=0$$当且仅当$$\vec{v}=0$$
  - 对所有$$a\in\mathbf{F}, \vec{v}\in V$$都有$$\| a\vec{v}\|=\vert a\vert\| \vec{v}\|$$（两边平方即可证）
- 对于两个向量$$\vec{u},\vec{v}\in V$$，如果$$\langle\vec{u},\vec{v}\rangle=0$$，则称$$\vec{u}$$和$$\vec{v}$$是正交的
  - 注意次序无关紧要（由共轭对称性）
...
- 6.5：正交分解：设$$\vec{u},\vec{v}\in V$$，如果$$\vec{v}\ne0$$，可以证明这个式子把$$\vec{u}$$写成了$$\vec{v}$$的标量倍加上一个正交于$$\vec{v}$$的向量：
  ...
  $$
  \vec{u}=\frac{\langle\vec{u},\vec{v}\rangle}{\|\vec{v}\|^{2}}\vec{v}+\left(\vec{u}-\frac{\langle\vec{u},\vec{v}\rangle}{\|\vec{v}\|^{2}}\vec{v}\right)
  $$
- 6.6：柯西-施瓦茨不等式（Cauchy-Schwarz Inequality）：若$$\vec{u},\vec{v}\in V$$，则

  $$
  \vert \langle\vec{u},\vec{v}\rangle\vert\le\| \vec{u}\|\| \vec{v}\|
  $$
  ...
  - 证明：设$$\vec{v}\ne\vec{0}$$（否则等号成立），然后用上述正交分解。令$$\vec{w}$$等于右端第二项，则由勾股定理有：
    ...
    $$
    \| \vec{u}\|^{2}=\left\| \frac{\langle\vec{u},\vec{v}\rangle}{\| \vec{v}\|^{2}}\vec{v}\right\|^{2}+\| \vec{w}\|^{2}=\frac{\vert\langle\vec{u},\vec{v}\rangle\vert^{2}}{\|\vec{v}\|^{2}}+\|\vec{w}\|^{2}\ge\frac{\vert \langle\vec{u},\vec{v}\rangle\vert^{2}}{\|\vec{v}\|^{2}}
    $$
...
- 6.9：三角不等式（Triangle Inequality）：若$$\vec{u},\vec{v}\in V$$则$$\|\vec{u}+\vec{v}\|\le\vec{u}+\vec{v}$$，而且其中的等号成立当且仅当$$\vec{u},\vec{v}$$之一是另一个的非负标量（实数）倍。
  - 证明：展开可得：
    ...
    $$
    \|\vec{u}+\vec{v}\|=\|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\text{Re}\langle\vec{u},\vec{v}\rangle\le \|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\vert \langle\vec{u},\vec{v}\rangle\vert \le\|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\|\vec{u}\|\|\vec{v}\|=(\|\vec{u}\|+\|\vec{v}\|)^{2}
    $$
...
- 6.15：如果$$V$$中的向量组$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$是规范正交的，那么$$\|a_{1}\vec{e}_{1}+\cdots+a_{m}\vec{e}_{m}\|^{2}=\vert a_{1}\vert^{2}+\cdots+\vert a_{m}\vert^{2}$$，其中$$a_{1},\cdots,a_{m}\in\mathbf{F}$$。证明：反复使用勾股定理6.3以及上述范数的第二个性质即得。
- 6.16：推论：每个规范正交向量组都是线性无关的。证明：用线性无关的定义（若存在一组标量使该规范正交向量组和这组标量对应项的积的和为0，那么这组标量全为0），根据6.15即得。
- 6.17：定理：设$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，则对每个$$\vec{v}\in V$$都有：
  - 6.18：$$\vec{v}=\langle\vec{v},\vec{e}_{1}\rangle\vec{e}_{1}+\cdots+\langle\vec{v},\vec{e}_{n}\rangle\vec{e}_{n}$$，而且
  - 6.19：$$\|\vec{v}\|^{2}=\vert \langle\vec{v},\vec{e}_{1}\rangle\vert^{2}+\cdots+\vert \langle\vec{v},\vec{e}_{n}\rangle\vert^{2}$$
  - 证明：对等式$$\vec{v}=a_{1}\vec{e}_{1}+\cdots+a_{n}\vec{e}_{n}$$两端都与$$\vec{e}_{j}$$做内积，即得$$\langle\vec{v},\vec{e}_{j}\rangle=a_{j}$$，即6.18成立。又由6.15即得6.19。
...
- 正交补：如果$$U$$是$$V$$的子集，那么$$U$$的正交补（orthogonal complement）为：
  ...
  $$
  U^{\bot}=\{\vec{v}\in V:\langle\vec{v},\vec{u}\rangle=0, u\in U\}
  $$
...
- 6.29：定理：如果$$U$$是$$V$$的子空间，那么$$V=U\oplus U^{\bot}$$
  - 证明：先证$$V=U+U^{\bot}$$，为此设$$\vec{v}\in V$$且$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$是$$U$$的一个规范正交基，则有

    $$
    \vec{v}=\underbrace{\langle\vec{v},\vec{e}_{1}\rangle\vec{e}_{1}+\cdots+\langle\vec{v},\vec{e}_{m}\rangle\vec{e}_{m}}_{\vec{u}}+\underbrace{\vec{v}-\langle\vec{v},\vec{e}_{1}\rangle\vec{e}_{1}-\cdots-\langle\vec{v},\vec{e}_{m}\rangle\vec{e}_{m}}_{\vec{w}}
    $$

    显然$$\vec{u}\in U$$，易证对每个$$j$$都有$$\langle\vec{w},\vec{e}_{j}\rangle=0$$，因此$$\vec{w}\in U^{\bot}$$，证得。再证该和是直和，为此只需证$$U\cap U^{\bot}=\{\vec{0}\}$$，为此设$$\vec{v}$$属于该交集，则它（包含于$$U$$）正交于$$U$$中的每个向量（包括$$\vec{v}$$本身），从而$$\langle\vec{v},\vec{v}\rangle=0$$即$$\vec{v}=\vec{0}$$，即得。
...
- 6.35：根据6.29的证明所用的分解可知，如果$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$是$$U$$的规范正交基，那么对于每个$$\vec{v}\in V$$都有$$P_{U}\vec{v}=\langle\vec{v},\vec{e}_{1}\rangle\vec{e}_{1}+\cdots+\langle\vec{v},\vec{e}_{m}\rangle\vec{e}_{m}$$
...
- 6.45：定理：设$$\varphi$$是$$V$$上的线性泛函，则存在唯一一个向量$$\vec{v}\in V$$使得$$\varphi(\vec{u})=\langle\vec{u},\vec{v}\rangle,\vec{u}\in V$$
  - 证明：首先证明存在向量$$\vec{v}\in V$$使得对每个$$\vec{u}\in V$$都有$$\varphi(\vec{u})=\langle\vec{u},\vec{v}\rangle$$。设$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，则对每个$$\vec{u}\in V$$都有：

    $$
    \begin{array}{l}
      \varphi(\vec{u})&=\varphi\left(\langle\vec{u},\vec{e}_{1}\rangle\vec{e}_{1}+\cdots+\langle\vec{u},\vec{e}_{n}\rangle\vec{e}_{n}\right) \\
      &=\langle\vec{u},\vec{e}_{1}\rangle\varphi(\vec{e}_{1})+\cdots+\langle\vec{u},\vec{e}_{n}\rangle\varphi(\vec{e}_{n}) \\
      &=\langle\vec{u},\overline{\varphi(\vec{e}_{1})}\vec{e}_{1}+\cdots+\overline{\varphi(\vec{e}_{n})}\vec{e}_{n}\rangle \\
      &=\langle\vec{u},\vec{v}\rangle
    \end{array}
    $$

    现在来证明这样的$$\vec{v}$$只有一个，否则假设$$\vec{v}_{1},\vec{v}_{2}$$对任意的$$\vec{u}$$均满足条件，则取$$\vec{u}=\vec{v}_{1}-\vec{v}_{2}$$，易证$$\vec{v}_{1}=\vec{v}_{2}$$。
- 伴随（adjoint）的定义：设$$V,W$$是$$\mathbf{F}$$上的有限维非零内积空间，设$$T\in\mathcal{L}(V,W)$$。给定$$\vec{w}\in W$$，考虑$$V$$上将$$\vec{v}\in V$$映成$$\langle T\vec{v},\vec{w}\rangle$$的线性泛函$$\varphi_{\vec{w}}$$（易证这个线性泛函满足线性映射的定义，因此是良定义的），则$$T$$的伴随$$T^{*}$$是使得如下性质成立的从$$W$$到$$V$$的函数：$$T^{*}w$$是$$V$$中唯一一个满足条件$$\langle T\vec{v},\vec{w}\rangle=\langle\vec{v},T^{*}\vec{w}\rangle,\vec{v}\in V$$的向量（该向量的存在性和唯一性由6.45得到）
  - 注意：伴随是一个函数，其定义域为$$W$$，而上述描述定义了它在每个$$\vec{w}\in W$$上的值，该值满足：对任何$$\vec{v}\in V$$成立等式$$\langle T\vec{v},\vec{w}\rangle=\langle\vec{v},T^{*}\vec{w}\rangle$$
- 伴随是线性映射：若$$T\in\mathcal{L}(V,W)$$，则$$T^{*}\in\mathcal{L}(V,W)$$。证明：根据线性映射的定义需要证明加性和齐性。
  - 加性：$$\langle\vec{v},T^{*}(\vec{w}_{1}+\vec{w}_{2})\rangle=\langle T\vec{v},\vec{w}_{1}+\vec{w}_{2}\rangle=\langle T\vec{v},\vec{w}_{1}\rangle+\langle T\vec{v},\vec{w}_{2}\rangle=\langle\vec{v},T^{*}\vec{w}_{1}\rangle+\langle\vec{v},T^{*}\vec{w}_{2}\rangle=\langle\vec{v},T^{*}\vec{w}_{1}+T^{*}\vec{w}_{2}\rangle$$
  - 齐性可以类似地证明。
- 函数$$T\mapsto T^{*}$$具有如下性质（以下设$$S,T\in\mathcal{L}(V,W)$$）
  - 加性：$$(S+T)^{*}=S^{*}+T^{*}$$。为证此性质需要证明$$(S+T)^{*}(\vec{w})=S^{*}\vec{w}+T^{*}\vec{w},\vec{w}\in W$$，为此需要证明$$\langle\vec{v},(S+T)^{*}\vec{w}\rangle=\langle\vec{v},S^{*}\vec{w}+T^{*}\vec{w}\rangle$$，而这通过伴随定义的式子、内积的性质以及线性变换的性质就能证得。
...
- 6.46：命题：设$$T\in\mathcal{L}(V,W)$$，则
  ...
  - 证明：对于(a)，设$$\vec{w}\in W$$，则：
    ...
    &\iff \langle\vec{v},T^{*}\vec{w}\rangle=0,\vec{v}\in V \\
    &\iff \langle T\vec{v},\vec{w}\rangle=0,\vec{v}\in V \\
    &\iff \vec{w}\in (\text{range}T)^{\bot}
    \end{array}
    $$

    (a)的两端取正交补即得(d)（用到了6.33）。通过把这两式中的$$T$$和$$T^{*}$$互换即得另外两式。
- 6.47：命题：设$$T\in\mathcal{L}(V,W)$$，如果$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，且$$(\vec{f}_{1},\cdots,\vec{f}_{m})$$是$$W$$的规范正交基，那么

  $$
  \mathcal{M}(T^{*},(\vec{f}_{1},\cdots,\vec{f}_{m}),(\vec{e}_{1},\cdots,\vec{e}_{n}))
  $$

  是

  $$
  \mathcal{M}(T,(\vec{e}_{1},\cdots,\vec{e}_{n}),(\vec{f}_{1},\cdots,\vec{f}_{m}))
  $$

  的共轭转置。
  - 证明：设$$(\vec{e}_{1},\cdots,\vec{e}_{n}),(\vec{f}_{1},\cdots,\vec{f}_{m})$$分别是$$V,W$$的规范正交基。我们知道，把$$T\vec{e}_{k}$$写成这些$$\vec{f}_{j}$$的线性组合可以得到$$\mathcal{M}(T)$$的第$$k$$列，而根据6.17有：

    $$
    T\vec{e}_{k}=\langle T\vec{e}_{k},\vec{f}_{1}\rangle\vec{f}_{1}+\cdots+\langle T\vec{e}_{k},\vec{f}_{m}\rangle\vec{f}_{m}
    $$

    因此$$\mathcal{M}(T)$$中第$$j$$行第$$k$$列的元素是$$\langle T\vec{e}_{k},\vec{f}_{j}\rangle$$。类似地$$\mathcal{M}(T^{*})$$中第$$j$$行第$$k$$列的元素是$$\langle T^{*}\vec{f}_{k},\vec{e}_{j}\rangle$$，而我们有：

    $$
    \langle T^{*}\vec{f}_{k},\vec{e}_{j}\rangle=\langle\vec{f}_{k},T\vec{e}_{j}\rangle=\overline{\langle T\vec{e}_{j},\vec{f}_{k}\rangle}
    $$
...
- 7.1：命题：自伴算子的特征值都是实的。证明：思路就是证明$$\lambda=\overline{\lambda}$$：
  ...
  $$
  \begin{array}{l}
  \lambda\|\vec{v}\|^{2}=\langle\lambda\vec{v},\vec{v}\rangle=\langle T\vec{v},\vec{v}\rangle=\langle\vec{v},T\vec{v}\rangle=\langle\vec{v},\lambda\vec{v}\rangle=\overline{\lambda}\|\vec{v}\|^{2}
  \end{array}
  $$
- 7.2：命题：若$$V$$是复内积空间，$$T$$是$$V$$上的算子，使得对所有$$\vec{v}\in V$$都有$$\langle T\vec{v},\vec{v}\rangle=0$$，则$$T=0$$
  - 证明：思路就是用$$\langle T\vec{v},\vec{v}\rangle$$的形式来表示$$\langle T\vec{u},\vec{w}\rangle$$：

    $$
    \begin{align}
      \langle T\vec{u},\vec{w}\rangle &=\frac{2\langle T\vec{u},\vec{w}\rangle+2\langle T\vec{w},\vec{u}\rangle}{4} + \frac{2\langle T\vec{u},i\vec{w}\rangle+2\langle T(i\vec{w}),\vec{u}\rangle}{4}i \\
      &=\frac{\langle T(\vec{u}+\vec{w}),\vec{u}+\vec{w}\rangle-\langle T(\vec{u}-\vec{w}),\vec{u}-\vec{w}\rangle}{4}+\frac{\langle T(\vec{u}+i\vec{w}),\vec{u}+i\vec{w}\rangle-\langle T(\vec{u}-i\vec{w}),\vec{u}-i\vec{w}\rangle}{4}i \\
      &=0
    \end{align}
    $$
...
- 7.3：推论：设$$V$$是复内积空间，$$T\in\mathcal{L}(V)$$，则$$T$$是自伴的当且仅当对每个$$\vec{v}\in V$$都有$$\langle T\vec{v},\vec{v}\rangle\in \mathbf{R}$$
  - 证明：

    $$
    \begin{align}
      \langle T\vec{v},\vec{v}\rangle-\overline{\langle T\vec{v},\vec{v}\rangle}&=\langle T\vec{v},\vec{v}\rangle-\langle\vec{v}, T\vec{v}\rangle \\
      &=\langle T\vec{v},\vec{v}\rangle-\langle T^{*}\vec{v},\vec{v}\rangle \\
      &=\langle(T-T^{*})\vec{v},\vec{v}\rangle
    \end{align}
    $$

    - =>方向：若$$T$$是自伴的，则上式右端为0，从而左端的两者相等，即两者都为实数。
    - <=方向：若左端第一项为实数则左端为0，根据7.2知$$T-T^{*}=0$$，因此$$T$$是自伴的。
- 7.4：命题：若$$T$$是$$V$$上的自伴算子使得对所有$$\vec{v}\in V$$都有$$\langle T\vec{v},\vec{v}\rangle=0$$，则$$T=0$$
  - 证明：7.2已经证明了$$V$$是复内积空间的情况（且没有$$T$$是自伴的的假设），因此这里只需处理实内积空间的情况，这时有$$\langle T\vec{w},\vec{u}\rangle=\langle\vec{w},T\vec{u}\rangle=\langle T\vec{u},\vec{w}\rangle$$，而据此可得：

    $$
    \begin{align}
      \langle T\vec{u},\vec{w}\rangle &=\frac{2\langle T\vec{u},\vec{w}\rangle+2\langle T\vec{w},\vec{u}\rangle}{4} \\
      &=\frac{\langle T(\vec{u}+\vec{w}),\vec{u}+\vec{w}\rangle-\langle T(\vec{u}-\vec{w}),\vec{u}-\vec{w}\rangle}{4} \\
      &=0
    \end{align}
    $$
...
b       <a,b>: \langle\vec{u},\vec{v}\rangle
e      rangeT: \text{range}T
f       nullT: \text{null}T
p            : p(z)=a_{0}+a_{1}z+a_{2}z^{2}+\cdots+a_{m}z^{m}
l   lambda1-m: \lambda_{1},\cdots,\lambda_{m}
u       u1-um: (\vec{u}_{1},\cdots,\vec{u}_{m})
v       v1-vn: (\vec{v}_{1},\cdots,\vec{v}_{n})
w       w1-wm: (\vec{w}_{1},\cdots,\vec{w}_{m})
```
