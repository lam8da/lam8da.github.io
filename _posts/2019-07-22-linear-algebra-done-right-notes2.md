...
- 实向量空间或复向量空间$$V$$上的内积（inner product）就是一个函数，它把$$V$$中元素的每个有序对$$(\vec{u},\vec{v})$$都映成一个数$$\vinprod{u}{v}\in\mathbf{F}$$，并且具有性质：
  - 正性：对所有$$\vec{v}\in V$$都有$$\vinprod{v}{v}\ge0$$（这意味着$$\vinprod{v}{v}$$是一个实数）
  - 定性：$$\vinprod{v}{v}=0$$当且仅当$$\vec{v}=0$$
  - 第一个位置的加性：对所有$$\vec{u},\vec{v},\vec{w}\in V$$都有$$\inprod{\vec{u}+\vec{v}}{\vec{w}}=\vinprod{u}{w}+\vinprod{v}{w}$$
    - 通过这个和下一个性质可以证明第二个位置也具有加性，即：$$\inprod{\vec{u}}{\vec{v}+\vec{w}}=\vinprod{u}{v}+\vinprod{u}{w}$$
    - 可证$$\vinprod{0}{w}=\vinprod{w}{0}=0,\vec{w}\in V$$
  - 第一个位置的齐性：对所有$$a\in\mathbf{F},\vec{v},\vec{w}\in V$$都有$$\inprod{ a\vec{v}}{\vec{w}}=a\vinprod{v}{w}$$
    - 通过这个和上一个性质可以证明第二个位置具有共轭齐性，即：$$\inprod{\vec{u}}{a\vec{v}}=\overline{a}\vinprod{u}{v}$$
  - 共轭对称性：对所有$$\vec{v},\vec{w}\in V$$都有$$\vinprod{v}{w}=\overline{\vinprod{w}{v}}$$
    - 对实向量空间这等价于$$\vinprod{v}{w}=\vinprod{w}{v}$$
- 内积空间是带有内积的向量空间。例子：
  - $$\mathbf{F}^{n}$$，内积定义为$$\inprod{(w_{1},\cdots,w_{n})}{(z_{1},\cdots,z_{n})}=w_{1}\overline{z_{1}}+\cdots+w_{n}\overline{z_{n}}$$（这称为欧几里得内积）
  - 同样为$$\mathbf{F}^{n}$$，但用另一个内积：若$$c_{1},\cdots,c_{n}$$是正数，定义内积$$\inprod{(w_{1},\cdots,w_{n})}{(z_{1},\cdots,z_{n})}=c_{1}w_{1}\overline{z_{1}}+\cdots+c_{n}w_{n}\overline{z_{n}}$$
  - 由系数在$$\mathbf{F}$$中次数不超过$$m$$的所有多项式组成的向量空间$$\mathcal{P}_{m}(\mathbf{F})$$，定义内积$$\inprod{ p}{q}=\int_{0}^{1}p(x)\overline{q(x)}\mathrm{d}x$$

## 范数

- 范数定义为$$\| \vec{v}\|=\sqrt{\vinprod{v}{v}}$$。性质：
  - $$\| \vec{v}\|=0$$当且仅当$$\vec{v}=0$$
  - 对所有$$a\in\mathbf{F}, \vec{v}\in V$$都有$$\| a\vec{v}\|=\vert a\vert\| \vec{v}\|$$（两边平方即可证）
- 对于两个向量$$\vec{u},\vec{v}\in V$$，如果$$\vinprod{u}{v}=0$$，则称$$\vec{u}$$和$$\vec{v}$$是正交的
  - 注意次序无关紧要（由共轭对称性）
...
- 6.5：正交分解：设$$\vec{u},\vec{v}\in V$$，如果$$\vec{v}\ne0$$，可以证明这个式子把$$\vec{u}$$写成了$$\vec{v}$$的标量倍加上一个正交于$$\vec{v}$$的向量：
  ...
  $$
  \vec{u}=\frac{\vinprod{u}{v}}{\|\vec{v}\|^{2}}\vec{v}+\left(\vec{u}-\frac{\vinprod{u}{v}}{\|\vec{v}\|^{2}}\vec{v}\right)
  $$
- 6.6：柯西-施瓦茨不等式（Cauchy-Schwarz Inequality）：若$$\vec{u},\vec{v}\in V$$，则

  $$
  \vert \vinprod{u}{v}\vert\le\| \vec{u}\|\| \vec{v}\|
  $$
  ...
  - 证明：设$$\vec{v}\ne\vec{0}$$（否则等号成立），然后用上述正交分解。令$$\vec{w}$$等于右端第二项，则由勾股定理有：
    ...
    $$
    \| \vec{u}\|^{2}=\left\| \frac{\vinprod{u}{v}}{\| \vec{v}\|^{2}}\vec{v}\right\|^{2}+\| \vec{w}\|^{2}=\frac{\vert\vinprod{u}{v}\vert^{2}}{\|\vec{v}\|^{2}}+\|\vec{w}\|^{2}\ge\frac{\vert \vinprod{u}{v}\vert^{2}}{\|\vec{v}\|^{2}}
    $$
...
- 6.9：三角不等式（Triangle Inequality）：若$$\vec{u},\vec{v}\in V$$则$$\|\vec{u}+\vec{v}\|\le\vec{u}+\vec{v}$$，而且其中的等号成立当且仅当$$\vec{u},\vec{v}$$之一是另一个的非负标量（实数）倍。
  - 证明：展开可得：
    ...
    $$
    \|\vec{u}+\vec{v}\|=\|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\text{Re}\vinprod{u}{v}\le \|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\vert \vinprod{u}{v}\vert \le\|\vec{u}\|^{2}+\|\vec{v}\|^{2}+2\|\vec{u}\|\|\vec{v}\|=(\|\vec{u}\|+\|\vec{v}\|)^{2}
    $$
...
- 6.17：定理：设$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，则对每个$$\vec{v}\in V$$都有：
  - 6.18：$$\vec{v}=\inprod{\vec{v}}{\vec{e}_{1}}\vec{e}_{1}+\cdots+\inprod{\vec{v}}{\vec{e}_{n}}\vec{e}_{n}$$，而且
  - 6.19：$$\|\vec{v}\|^{2}=\vert \inprod{\vec{v}}{\vec{e}_{1}}\vert^{2}+\cdots+\vert \inprod{\vec{v}}{\vec{e}_{n}}\vert^{2}$$
  - 证明：对等式$$\vec{v}=a_{1}\vec{e}_{1}+\cdots+a_{n}\vec{e}_{n}$$两端都与$$\vec{e}_{j}$$做内积，即得$$\inprod{\vec{v}}{\vec{e}_{j}}=a_{j}$$，即6.18成立。又由6.15即得6.19。
- 6.20：格拉姆-施密特过程（Gram-Schmidt procedure）：如果$$(\vec{v}_{1},\cdots,\vec{v}_{m})$$是$$V$$中的线性无关向量组，则$$V$$有规范正交向量组$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$使得：
  ...
  - 证明：首先令$$\vec{e}_{1}=\vec{v}_{1}/\|\vec{v}_{1}\|$$，这满足$$j=1$$的情况。然后归纳地选取$$\vec{e}_{1},\cdots,\vec{e}_{m}$$。假设$$j>1$$并且已经选取了规范正交组$$(\vec{e}_{1},\cdots,\vec{e}_{j-1})$$使得$$\vspanr{v}{j-1}=\vspanr{e}{j-1}$$。现在令
    ...
    $$
    \vec{e}_{j}=\frac{\vec{v}_{j}-\inprod{\vec{v}_{j}}{\vec{e}_{1}}\vec{e}_{1}-\cdots-\inprod{\vec{v}_{j}}{\vec{e}_{j-1}}\vec{e}_{j-1}}{\|\vec{v}_{j}-\inprod{\vec{v}_{j}}{\vec{e}_{1}}\vec{e}_{1}-\cdots-\inprod{\vec{v}_{j}}{\vec{e}_{j-1}}\vec{e}_{j-1}\|}
    $$

    易知$$\vec{v}_{j}\notin\vspanr{e}{j-1}$$，因此上式良定义（分母不为0）且$$\|\vec{e}_{j}\|=1$$。设$$1\le k\le j$$，用上式将$$\inprod{\vec{e}_{j}}{\vec{e}_{k}}$$展开即得其值为0，因此$$(\vec{e}_{1},\cdots,\vec{e}_{j})$$是规范正交组。由上式知$$\vec{v}_{j}\in\vspanr{e}{j}$$，再根据归纳假设即得$$\vspanr{v}{j}\subset\vspanr{e}{j}$$，而由于这两个组都是线性无关的（后者根据6.16得到），因此上面两个子空间的维数都是$$j$$，从而一定相等。
...
- 正交补：如果$$U$$是$$V$$的子集，那么$$U$$的正交补（orthogonal complement）为：
  ...
  $$
  U^{\bot}=\{\vec{v}\in V:\vinprod{v}{u}=0, u\in U\}
  $$
...
- 6.29：定理：如果$$U$$是$$V$$的子空间，那么$$V=U\oplus U^{\bot}$$
  - 证明：先证$$V=U+U^{\bot}$$，为此设$$\vec{v}\in V$$且$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$是$$U$$的一个规范正交基，则有
    ...
    $$
    \vec{v}=\underbrace{\inprod{\vec{v}}{\vec{e}_{1}}\vec{e}_{1}+\cdots+\inprod{\vec{v}}{\vec{e}_{m}}\vec{e}_{m}}_{\vec{u}}+\underbrace{\vec{v}-\inprod{\vec{v}}{\vec{e}_{1}}\vec{e}_{1}-\cdots-\inprod{\vec{v}}{\vec{e}_{m}}\vec{e}_{m}}_{\vec{w}}
    $$

    显然$$\vec{u}\in U$$，易证对每个$$j$$都有$$\inprod{\vec{w}}{\vec{e}_{j}}=0$$，因此$$\vec{w}\in U^{\bot}$$，证得。再证该和是直和，为此只需证$$U\cap U^{\bot}=\{\vec{0}\}$$，为此设$$\vec{v}$$属于该交集，则它（包含于$$U$$）正交于$$U$$中的每个向量（包括$$\vec{v}$$本身），从而$$\vinprod{v}{v}=0$$即$$\vec{v}=\vec{0}$$，即得。
...
- 6.35：根据6.29的证明所用的分解可知，如果$$(\vec{e}_{1},\cdots,\vec{e}_{m})$$是$$U$$的规范正交基，那么对于每个$$\vec{v}\in V$$都有$$P_{U}\vec{v}=\inprod{\vec{v}}{\vec{e}_{1}}\vec{e}_{1}+\cdots+\inprod{\vec{v}}{\vec{e}_{m}}\vec{e}_{m}$$
- 6.36：命题：设$$U$$是$$V$$的子空间并且$$\vec{v}\in V$$，则$$\|\vec{v}-P_{U}\vec{v}\|\le\|\vec{v}-\vec{u}\|,\vec{u}\in U$$。进一步，若上面等号成立，则$$\vec{u}=P_{U}\vec{v}$$
  ...
  - 例子：如何找到一个次数不超过5的实系数多项式$$\vec{u}$$使其在区间$$[-\pi,\pi]$$上尽量好地逼近$$\sin x$$，即$$\int_{-\pi}^{\pi}\vert\sin x-\vec{u}(x)\vert^{2}\mathrm{d}x$$最小
    - 解：令$$C[-\pi,\pi]$$表示由$$[-\pi,\pi]$$上的连续实值函数组成的实向量空间（注意这个空间不是有限维的），并取内积为：
      ...
      $$
      \inprod{ f}{g}=\int_{-\pi}^{\pi}f(x)g(x)\mathrm{d}x
      $$
...
- 6.45：定理：设$$\varphi$$是$$V$$上的线性泛函，则存在唯一一个向量$$\vec{v}\in V$$使得$$\varphi(\vec{u})=\vinprod{u}{v},\vec{u}\in V$$
  - 证明：首先证明存在向量$$\vec{v}\in V$$使得对每个$$\vec{u}\in V$$都有$$\varphi(\vec{u})=\vinprod{u}{v}$$。设$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，则对每个$$\vec{u}\in V$$都有：

    $$
    \begin{array}{l}
      \varphi(\vec{u})&=\varphi\left(\inprod{\vec{u}}{\vec{e}_{1}}\vec{e}_{1}+\cdots+\inprod{\vec{u}}{\vec{e}_{n}}\vec{e}_{n}\right) \\
      &=\inprod{\vec{u}}{\vec{e}_{1}}\varphi(\vec{e}_{1})+\cdots+\inprod{\vec{u}}{\vec{e}_{n}}\varphi(\vec{e}_{n}) \\
      &=\inprod{\vec{u}}{\overline{\varphi(\vec{e}_{1})}\vec{e}_{1}+\cdots+\overline{\varphi(\vec{e}_{n})}\vec{e}_{n}} \\
      &=\vinprod{u}{v}
    \end{array}
    $$

    现在来证明这样的$$\vec{v}$$只有一个，否则假设$$\vec{v}_{1},\vec{v}_{2}$$对任意的$$\vec{u}$$均满足条件，则取$$\vec{u}=\vec{v}_{1}-\vec{v}_{2}$$，易证$$\vec{v}_{1}=\vec{v}_{2}$$。
- 伴随（adjoint）的定义：设$$V,W$$是$$\mathbf{F}$$上的有限维非零内积空间，设$$T\in\mathcal{L}(V,W)$$。给定$$\vec{w}\in W$$，考虑$$V$$上将$$\vec{v}\in V$$映成$$\inprod{ T\vec{v}}{\vec{w}}$$的线性泛函$$\varphi_{\vec{w}}$$（易证这个线性泛函满足线性映射的定义，因此是良定义的），则$$T$$的伴随$$T^{*}$$是使得如下性质成立的从$$W$$到$$V$$的函数：$$T^{*}w$$是$$V$$中唯一一个满足条件$$\inprod{ T\vec{v}}{\vec{w}}=\inprod{\vec{v}}{T^{*}\vec{w}},\vec{v}\in V$$的向量（该向量的存在性和唯一性由6.45得到）
  - 注意：伴随是一个函数，其定义域为$$W$$，而上述描述定义了它在每个$$\vec{w}\in W$$上的值，该值满足：对任何$$\vec{v}\in V$$成立等式$$\inprod{ T\vec{v}}{\vec{w}}=\inprod{\vec{v}}{T^{*}\vec{w}}$$
- 伴随是线性映射：若$$T\in\mathcal{L}(V,W)$$，则$$T^{*}\in\mathcal{L}(V,W)$$。证明：根据线性映射的定义需要证明加性和齐性。
  - 加性：$$\inprod{\vec{v}}{T^{*}(\vec{w}_{1}+\vec{w}_{2})}=\inprod{ T\vec{v}}{\vec{w}_{1}+\vec{w}_{2}}=\inprod{ T\vec{v}}{\vec{w}_{1}}+\inprod{ T\vec{v}}{\vec{w}_{2}}=\inprod{\vec{v}}{T^{*}\vec{w}_{1}}+\inprod{\vec{v}}{T^{*}\vec{w}_{2}}=\inprod{\vec{v}}{T^{*}\vec{w}_{1}+T^{*}\vec{w}_{2}}$$
  - 齐性可以类似地证明。
- 函数$$T\mapsto T^{*}$$具有如下性质（以下设$$S,T\in\mathcal{L}(V,W)$$）
  - 加性：$$(S+T)^{*}=S^{*}+T^{*}$$。为证此性质需要证明$$(S+T)^{*}(\vec{w})=S^{*}\vec{w}+T^{*}\vec{w},\vec{w}\in W$$，为此需要证明$$\inprod{\vec{v}}{(S+T)^{*}\vec{w}}=\inprod{\vec{v}}{S^{*}\vec{w}+T^{*}\vec{w}}$$，而这通过伴随定义的式子、内积的性质以及线性变换的性质就能证得。
...
- 6.46：命题：设$$T\in\mathcal{L}(V,W)$$，则
  ...
  - 证明：对于(a)，设$$\vec{w}\in W$$，则：
    ...
    &\iff \inprod{\vec{v}}{T^{*}\vec{w}}=0,\vec{v}\in V \\
    &\iff \inprod{ T\vec{v}}{\vec{w}}=0,\vec{v}\in V \\
...
- 6.47：命题：设$$T\in\mathcal{L}(V,W)$$，如果$$(\vec{e}_{1},\cdots,\vec{e}_{n})$$是$$V$$的规范正交基，且$$(\vec{f}_{1},\cdots,\vec{f}_{m})$$是$$W$$的规范正交基，那么
  ...
  - 证明：设$$(\vec{e}_{1},\cdots,\vec{e}_{n}),(\vec{f}_{1},\cdots,\vec{f}_{m})$$分别是$$V,W$$的规范正交基。我们知道，把$$T\vec{e}_{k}$$写成这些$$\vec{f}_{j}$$的线性组合可以得到$$\mathcal{M}(T)$$的第$$k$$列，而根据6.17有：
    ...
    $$
    T\vec{e}_{k}=\inprod{ T\vec{e}_{k}}{\vec{f}_{1}}\vec{f}_{1}+\cdots+\inprod{ T\vec{e}_{k}}{\vec{f}_{m}}\vec{f}_{m}
    $$

    因此$$\mathcal{M}(T)$$中第$$j$$行第$$k$$列的元素是$$\inprod{ T\vec{e}_{k}}{\vec{f}_{j}}$$。类似地$$\mathcal{M}(T^{*})$$中第$$j$$行第$$k$$列的元素是$$\inprod{ T^{*}\vec{f}_{k}}{\vec{e}_{j}}$$，而我们有：

    $$
    \inprod{ T^{*}\vec{f}_{k}}{\vec{e}_{j}}=\inprod{\vec{f}_{k}}{T\vec{e}_{j}}=\overline{\inprod{ T\vec{e}_{j}}{\vec{f}_{k}}}
    $$
...
- 7.1：命题：自伴算子的特征值都是实的。证明：思路就是证明$$\lambda=\overline{\lambda}$$：
  ...
  $$
  \begin{array}{l}
  \lambda\|\vec{v}\|^{2}=\inprod{\lambda\vec{v}}{\vec{v}}=\inprod{ T\vec{v}}{\vec{v}}=\inprod{\vec{v}}{T\vec{v}}=\inprod{\vec{v}}{\lambda\vec{v}}=\overline{\lambda}\|\vec{v}\|^{2}
  \end{array}
  $$
- 7.2：命题：若$$V$$是复内积空间，$$T$$是$$V$$上的算子，使得对所有$$\vec{v}\in V$$都有$$\inprod{ T\vec{v}}{\vec{v}}=0$$，则$$T=0$$
  - 证明：思路就是用$$\inprod{ T\vec{v}}{\vec{v}}$$的形式来表示$$\inprod{ T\vec{u}}{\vec{w}}$$：

    $$
    \begin{align}
      \inprod{ T\vec{u}}{\vec{w}} &=\frac{2\inprod{ T\vec{u}}{\vec{w}}+2\inprod{ T\vec{w}}{\vec{u}}}{4} + \frac{2\inprod{ T\vec{u}}{i\vec{w}}+2\inprod{ T(i\vec{w})}{\vec{u}}}{4}i \\
      &=\frac{\inprod{ T(\vec{u}+\vec{w})}{\vec{u}+\vec{w}}-\inprod{ T(\vec{u}-\vec{w})}{\vec{u}-\vec{w}}}{4}+\frac{\inprod{ T(\vec{u}+i\vec{w})}{\vec{u}+i\vec{w}}-\inprod{ T(\vec{u}-i\vec{w})}{\vec{u}-i\vec{w}}}{4}i \\
      &=0
    \end{align}
    $$
...
- 7.3：推论：设$$V$$是复内积空间，$$T\in\mathcal{L}(V)$$，则$$T$$是自伴的当且仅当对每个$$\vec{v}\in V$$都有$$\inprod{ T\vec{v}}{\vec{v}}\in \mathbf{R}$$
  - 证明：

    $$
    \begin{align}
      \inprod{ T\vec{v}}{\vec{v}}-\overline{\inprod{ T\vec{v}}{\vec{v}}}&=\inprod{ T\vec{v}}{\vec{v}}-\inprod{\vec{v}}{ T\vec{v}} \\
      &=\inprod{ T\vec{v}}{\vec{v}}-\inprod{ T^{*}\vec{v}}{\vec{v}} \\
      &=\inprod{(T-T^{*})\vec{v}}{\vec{v}}
    \end{align}
    $$

    - =>方向：若$$T$$是自伴的，则上式右端为0，从而左端的两者相等，即两者都为实数。
    - <=方向：若左端第一项为实数则左端为0，根据7.2知$$T-T^{*}=0$$，因此$$T$$是自伴的。
- 7.4：命题：若$$T$$是$$V$$上的自伴算子使得对所有$$\vec{v}\in V$$都有$$\inprod{ T\vec{v}}{\vec{v}}=0$$，则$$T=0$$
  - 证明：7.2已经证明了$$V$$是复内积空间的情况（且没有$$T$$是自伴的的假设），因此这里只需处理实内积空间的情况，这时有$$\inprod{ T\vec{w}}{\vec{u}}=\inprod{\vec{w}}{T\vec{u}}=\inprod{ T\vec{u}}{\vec{w}}$$，而据此可得：

    $$
    \begin{align}
      \inprod{ T\vec{u}}{\vec{w}} &=\frac{2\inprod{ T\vec{u}}{\vec{w}}+2\inprod{ T\vec{w}}{\vec{u}}}{4} \\
      &=\frac{\inprod{ T(\vec{u}+\vec{w})}{\vec{u}+\vec{w}}-\inprod{ T(\vec{u}-\vec{w})}{\vec{u}-\vec{w}}}{4} \\
      &=0
    \end{align}
    $$
