...
- 定义：设$$D$$为无界区域，$$f(x,y)$$为其上定义的二元函数，对平面上任一包围原点的光滑封闭曲线$$\gamma$$，$$f(x,y)$$在$$\gamma$$所围的有界区域$$E_\gamma$$与$$D$$的交集$$E_\gamma\cap D=D_\gamma$$上恒可积。令$$d_\gamma=\min\{\sqrt{x^2+y^2}\vert(x,y)\in\gamma\}$$。若极限$$\lim_{d_\gamma\to \infty}\int_{D_\gamma}f(x,y)\mathrm{d}\sigma$$存在且有限且与$$\gamma$$的取法无关，则称$$f(x,y)$$在$$D$$上的反常二重积分收敛，并将该极限记为该反常二重积分的值。
- 定理21.17：反常二重积分收敛的一个充分条件，关于上下确界。
- 定理21.18：若无界区域$$D$$上$$f(x,y)\ge0$$，则反常二重积分收敛的充要条件是，在$$D$$的任何有界子区域上$$f(x,y)$$可积，且积分值有上界。
- 定理21.19：函数$$f(x,y)$$在无界区域$$D$$上的反常二重积分收敛的充要条件是$$\vert f(x,y) \vert$$在$$D$$上的反常二重积分收敛。充分性很容易证明，必要性书上让看菲赫金哥尔茨的教程WTF？
- 定理21.20（柯西判别法）：略

### 无界函数的二重积分

- 定义：函数在某有界区域除了其某个聚点外皆有定义，且在该聚点的任何空心邻域无解，$$\Delta$$为区域中任意包含该聚点的小区域，$$f(x,y)$$在$$D-\Delta$$上可积，若$$\lim_{d\to 0} \iint_{D-\Delta}f(x,y)\mathrm{d}\sigma$$存在且有限且与$$\Delta$$的取法无关，则称$$f(x,y)$$在$$D$$上的反常二重积分收敛，该极限值记为该反常二重积分的值。其中$$d$$表示$$\Delta$$的直径。
- 定理21.21（柯西判别法）：略
...
- 第一型曲面积分：设$$S$$是空间中可求面积的曲面，$$f(x,y,z)$$为定义在$$S$$上的函数，对$$S$$作分割$$T$$，它将$$S$$分为$$n$$个小曲面块$$S_i(i=1,2,\cdots,n)$$，以$$\Delta S_i$$记小曲面块$$S_i$$的面积，分割$$T$$的细度$$\Vert T\Vert=\max_{1\le i\le n}\{S_i的直径\}$$，在$$S_i$$上任取一点$$(\xi_i,\eta_i,\zeta_i)(i=1,2,\cdots,n)$$，若极限$$\lim_{\Vert T\Vert\to 0} \sum_{i=1}^{n}f(\xi_i,\eta_i,\zeta_i)\Delta S_i$$存在，且与分割$$T$$及$$(\xi_i,\eta_i,\zeta_i)$$的取法无关，则称此极限为$$f(x,y,z)$$在$$S$$上的第一型曲面积分，记为

  $$
  \iint_S f(x,y,z)\mathrm{d}S
  $$

- 第一型曲面积分的性质完全类似于第一形曲线积分，书里略去
- 定理22.1（用二重积分来计算第一型曲面积分）：设有光滑曲面$$S:z=z(x,y),(x,y)\in D$$，$$f(x,y,z)$$为$$S$$上的连续函数，则$$\iint_S f(x,y,z)\mathrm{d}S=\iint_D f(x,y,z(x,y))\sqrt{1+z_x^2+z_y^2}\mathrm{d}x\mathrm{d}y$$。证明过程和20.1类似。
- 和重积分类似，可以利用第一型曲面积分求出曲面块的质心、转动惯量、引力等。
- 定义：双侧曲面，单侧曲面（如莫比乌斯带)。书上用的是非严格定义。。。
- 对于由$$z=z(x,y)$$表示的曲面，通常以其法线正方向与$$z$$轴正向夹角成锐角的一侧为正侧。对封闭曲面，通常规定外侧为正侧。
