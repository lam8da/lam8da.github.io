...
1. 由上下两条曲线$$y=f_x(x),y=f_1(x)$$以及两条直线$$x=a,x=b,(a\lt b)$$所围的平面图形面积为$$\int_a^b\big(f_2(x)-f_1(x)\big)\dx$$
...
1. 定理11.1（无穷积分收敛的柯西准则）：无穷积分收敛的充要条件是任给$$\varepsilon>0$$，存在$$G\ge a$$，只要$$u_1,u_2>G$$便有$$\abs{\int_a^{u_2}f(x)\dx-\int_a^{u_1}f(x)\dx}=\abs{\int_{u_1}^{u_2}f(x)\dx}\lt\varepsilon$$。证明：通过函数极限的柯西准则。
