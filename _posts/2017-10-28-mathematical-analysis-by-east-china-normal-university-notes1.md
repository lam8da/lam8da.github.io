...
1. 一致连续性定理（定理4.9）：通过致密性定理证明，用反证法，存在$$\varepsilon_0$$，使得对任意的$$\delta$$，都存在$$x'$$和$$x''$$，即使$$\vert x'-x''\vert\lt\delta$$，但有$$\vert f(x')-f(x'')\vert\ge\varepsilon_0$$。令$$\delta=\frac{1}{n}$$，得出数列$$\{x'\}$$与$$\{x''\}$$，根据致密性定理，它们存在收敛子列，可证俩收敛子列收敛于同一点，但根据假设函数在俩子列对应点上的值的差不能小于$$\varepsilon_0$$，这跟$$f$$是连续函数矛盾
